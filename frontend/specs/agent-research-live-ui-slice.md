...
35. polling responseはdraft、draftMode、currentAttemptEpoch、currentGeneration、lastProcessedEventIdを
    更新しない。既存`recentEvents`は既知activityとしてruntime検証し、queued / runningのprogress適用可否と
    同じconnection state規則で`currentActivity`へ投影する。
36. `connecting`かつSSE event受理前はpolling progressを適用してよい。`live` / `reconnecting`では
    queued / runningのpolling progressを無視し、`polling-only`では適用する。`finalizing` / `terminal`では
    遅延したqueued / running responseを無視する。pollingのcompleted / failedはDB正本としてepochに依存せず
    終端判定へ渡す。queued stateで検証済み非terminal SSE eventを受理した場合は、DB acquire後のeventであるため
    `runStatus`をrunningへ単調に進め、stageとdraftを表示可能にする。
...
- EventSourceと2秒pollingが同一active runで並行し、pollingがSSE由来のdraftや境界stateを上書きしない。
- connectingのSSE受理前とpolling-onlyでは検証済み`recentEvents`から最新の関連activityを表示し、古いSSE
  activityを置換する。live / reconnectingでは遅いpolling activityで巻き戻さない。
...
SSEが`live`でもpolling自体は止めずcompleted / failedの検知を続ける。ただしactive statusのpolling responseで、
SSEから得たdraft、epoch、generation、Stream IDを変更しない。progress統合は次に固定する。
polling `recentEvents`はSSE activityと同じallowlist / camelCase field規則で検証し、unknown、schema違反、
snake_case payloadを捨てる。planning / nullでは最新`question.resolved`、retrievingでは最新検索activity、
synthesizingではactivityなしを選び、適用可能なstateでは`currentActivity`を置換する。

| connection state | queued / running polling progress | polling terminal |
|---|---|---|
| `connecting`かつSSE event受理前 | 適用可 | 受理 |
| `live` | 無視 | 受理 |
| `reconnecting` | 無視 | 受理 |
| `polling-only` | 適用 | 受理 |
...
- `connecting`のSSE event前と`polling-only`ではpolling progressを適用する。
- connecting / polling-onlyでは検証済み`recentEvents`の最新関連activityを適用し、polling-onlyでは旧SSE
  activityを置換する。live / reconnectingではpolling activityを適用しない。
- SSEでsynthesizingを受けた後、`live` / `reconnecting`中の遅いpolling retrievingで巻き戻さない。
