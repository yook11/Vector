...
- features の API 関数は `lib/api/server-fetcher.ts` (`serverFetch` / `publicServerFetch`) を経由 (Server Component / Server Action 専用)
...
├── lib/
│   ├── api/              # server-fetcher / fetcher / error / internal-config
