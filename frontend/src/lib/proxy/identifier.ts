 ...
 *
 * production の信頼構造は入口 (ALB は自身が XFF 末尾へ追記した値) に固有のため、
 * 推測せず CLIENT_IP_TRUST で明示宣言させる。未宣言は fail-closed (null) とし、
 ...
 *   左側の値は client が自由に書けるため読まない。
 * - dev / test は信頼境界が無いため trust を無視し、手元ツール向けの
 *   fallback (XFF 先頭 → x-real-ip) を維持する。
 */
export function extractClientIp({
  ...
  return asClientIp(forwardedFor?.split(",")[0]) ?? asClientIp(realIp);
}
