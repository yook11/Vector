...
分析トリオ 3 stage (curation / assessment / embedding) の ``audit_repository`` が
失敗経路の ``code`` / ``category`` を導出するときに共通で使う。
...
は SQLAlchemy 例外を明示 isinstance で分類し、queryable な ``code`` / ``cause`` の
ペアを返す (``.code`` を信用するのは各 stage の自前 marker だけにする方針)。

``code`` は ``pipeline_events.code`` 列に永続化される実質 wire 値なので、``cause``
...
class DbErrorCause(StrEnum):
    """DB 例外の性質。各 stage が ``Layer1Category`` への変換に使う中間表現。"""
...
class DbErrorClassification:
    ...
    Attributes:
        code: ``pipeline_events.code`` に焼く集計ラベル (wire 値、enum 非依存で固定)。
        cause: DB 例外の性質。各 stage が ``Layer1Category`` へ変換する軸。
    """
