...
- snapshot 不在は 200 + 全 null + 空 categories で返す。「まだ生成されていない」
  は故障ではないため、ステータスコードでは表現しない
...
async def get_weekly_trends(
    ...
    """最新週の weekly trends snapshot を返す (なければ空状態)。"""
    snapshot = await service.find_latest()
    if snapshot is None:
        return WeeklyTrendsResponse.empty()
    bundle = WeeklyTrendsBundle.model_validate(snapshot.bundle)
    return WeeklyTrendsResponse.from_snapshot(
        bundle=bundle,
