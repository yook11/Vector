...
ドメイン (Ready) と AI 層 (``InScope`` / ``OutOfScope``) を結び、判定実行 →
永続化 (楽観的ロック) → 勝者は audit + commit / 敗者は短絡の順序を担う。
...
class AssessmentService:
    ...
    対応する Repository に渡して永続化する。Domain Entity は介在せず、
    ...
    async def execute(
        ...
        async with self._session_factory() as session:
            match response:
                case InScope():
                    saved = await InScopeRepository(session).save(
                        response,
                        ready=ready,
                ...
                case OutOfScope():
                    assessment_id = await OutOfScopeRepository(session).save(
                        response,
                        ready=ready,
                        ai_model=assessor.model_name,
                    )
