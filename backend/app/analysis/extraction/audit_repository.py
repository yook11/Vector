...
ため、``extractor: BaseExtractor`` を引数で受け、``extractor.MODEL`` /
``extractor.PROMPT_VERSION`` から ``ai_model`` / ``prompt_version`` を埋める
(PR2 で envelope/extractor 化完了、Gemini ClassVar への静的依存を完全除去)。
"""
...
class ExtractionAuditRepository:
    ...
    async def append_drop_article(
        ...
        *,
        ...
        埋める (PR2、Gemini hardcode 依存を完全除去)。
        """
