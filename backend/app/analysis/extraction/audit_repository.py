...
- ``append_extracted`` / ``append_noise`` は成功 audit (caller が
  ...
  ``ai_model`` / ``prompt_version`` / ``raw_relevance`` は envelope
...
PR1-a スコープ外: 失敗経路 (``append_drop_article`` / ``append_failure``) の
envelope 化は PR2 で行う。現状は ``ai_model`` / ``prompt_version`` を
Gemini ClassVar から個別に埋める。
"""
...
class ExtractionAuditRepository:
    ...
    async def append_extracted(
        ...
        *,
        ...
        渡す (例: ``"extracted"``)。``envelope`` は ``ExtractionCall[Signal]``
        に narrow され、Service が ``match`` で振り分けた後にのみ呼ばれる。
        """
    ...
    async def append_drop_article(
        ...
        *,
        ...
        PR1-a スコープ外 (PR2 で envelope 化予定): 失敗経路は envelope を
        持たない (AI 呼び出し前 or 中の失敗) ため、``ai_model`` /
        ``prompt_version`` は Gemini ClassVar から直接埋める。
        """
