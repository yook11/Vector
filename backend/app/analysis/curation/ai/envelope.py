"""``CurationCall`` — curator 戻り値 envelope (PEP 695 Generic)。
...
PEP 695 Generic で ``CurationCall[Signal]`` / ``CurationCall[Noise]`` として
narrow し、``match call: case CurationCall(result=Signal())`` 経由で
container 単位の型 narrowing を効かせる (Stage 4 ``AssessmentCall[T]`` と対称)。

raw_response は curation 監査の S 級情報 (Vector のどこにも残らない極めて
貴重なデバッグ情報、`docs/observability/pipeline-events-design.md` 参照)。
prompt_version は ADR §prompt_version の規律で確定する 8 文字 hash で、
curator 自身が宣言した値を Service にそのまま伝搬する。
"""
...
from app.analysis.curation.domain import CurationResult


@dataclass(frozen=True, slots=True)
class CurationCall[T: CurationResult]:
    """curator の 1 回の API call の結果。

    ``T`` は ``Signal`` / ``Noise`` のいずれかに narrow される。Service 層は
    ``match call: case CurationCall(result=Signal()):`` で container ごと型を
    絞り、対応する Repository method (``CurationRepository.save_signal`` /
    ``CurationRepository.save_noise``) に envelope を 1 つ渡すだけで業務
    INSERT + audit 焼付を完結させる。

    Attributes:
        result: ドメイン詰め替え済みの curation 結果 (``Signal`` | ``Noise``)。
        ...
        model_name: ``BaseCurator.model_name`` property の値。Repository /
