...
``GeminiCurationResponse`` (Gemini SDK 契約型、``relevance`` 持ち) を受け取り、
``relevance`` 値で ``Signal`` / ``Noise`` ドメイン型に振り分ける。本関数が AI
出力のドメイン境界を 1 箇所に集約する (Stage 4 ``parse_assessment`` と対称)。

``GeminiCurationResponse`` 側で sanitize / not-empty / dedupe validators が
...
def parse_curation(response: GeminiCurationResponse) -> Signal | Noise:
    """``GeminiCurationResponse`` を ``Signal`` / ``Noise`` に振り分ける。

    ``response.relevance`` で振り分け、``GeminiCurationResponse`` 側 validator
