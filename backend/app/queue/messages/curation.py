...
class CurationTrigger(BaseModel):
    ...
    precondition は保証せず ``analyzable_article_id`` (A 空間 = analyzable_articles.id)
    のみを運ぶ。下流 Stage 3 Task が ``ReadyForCuration.try_advance_from`` を呼んで
    処理開始時に最新の DB 状態から Ready を構築する (案 3 = 厚い Ready + 下流 Stage
    自身が処理開始時に構築)。
    ...
    Deploy 方針: wire field を ``article_id`` から ``analyzable_article_id`` に改名
    した際、互換 alias は持たない。旧 ``article_id`` キーの in-flight message は
    required field 欠落で validation 拒否されるため、curation queue を drain した上で
    stop-the-world deploy する (旧 message を残さない)。
    """
