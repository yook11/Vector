"""共通broker catalogのschedulerがcron taskを発見するための副作用import集約点。

scheduler entrypoint (``app.queue.scheduler_entrypoint``) は本 module を import し、
共通catalogに残るcron taskのdecoratorをimport時に登録させる
(``broker.get_all_tasks()`` を満たし LabelScheduleSource が schedule label を回収できる
状態にする)。Trend Discoveryは機能側scheduler factoryが明示登録する。
"""

from __future__ import annotations

# cron 駆動 task を含む module を列挙する。
# scheduler の LabelScheduleSource が schedule label を回収する
...
import app.queue.tasks.backfill  # noqa: F401  (backfill_curations/assessments)
