...
class EmbeddingConsumerSettings(DatabaseConnectionSettings):
    ...
    def _require_production_iam(self) -> Self:
        if self.env == "production" and not self.db_iam_auth:
            raise ValueError("production Consumer requires RDS IAM authentication")
        return self
