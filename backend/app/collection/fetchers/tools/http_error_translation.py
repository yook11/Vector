...
- ``translate_fetch_exception`` は httpx / SSRF guard 例外を
  ...
  先に判定する。
- ``RECOVERABLE_FETCH_ERRORS`` は per-feed skip (cornell / nasa) と将来の marker
  分割が共有する「再試行で回復しうる」origin error tuple の単一 SSoT。旧
  ``TemporaryFetchError`` 発生条件の忠実訳。
"""
...
425 / 未分類 status / transport timeout / network) の忠実訳。bubble 側 (source
全体失敗として伝播させる) は AccessDenied / LegalBlock / ResourceNotFound /
Ssrf / Robots* / Redirect* / ResponseTooLarge / ContentTypeMismatch / Parse。
cornell / nasa の per-feed skip がこの tuple を ``except`` する。
"""
