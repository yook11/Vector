"""Electrek 用 Fetcher — Pattern R+H (Pattern H 設計で実装)。
...
- ``<category>`` を tags に採用 (実観察では "News" 1 個)
- ``<media:>`` namespace 未提供 → image_url=None 直書き
- ``<guid isPermaLink="false">`` (``?p=<id>`` 形式) を採用
- language は feed-level "en-US"

CleanTechnica と構造同型 (per-source 設計値が同じ、``NAME`` と
``ENDPOINT_URL`` ClassVar だけ差分。共通基底化はしない — memory
`feedback_phase_pr_split_pattern.md`)。
"""
...
class ElectrekFetcher:
    """Electrek 用 Pattern H Fetcher (Pattern R+H = HTML 必須)。"""
