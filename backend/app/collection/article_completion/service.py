...
- ``_load`` で pending を SELECT (``url`` 直接保持)、``status='running'``
  ガードで at-least-once 重複配送を静かに弾く
- ``_resolve_ready`` で HTML 取得 + ``ExtractionEmpty`` 判定 + promotion を
  実行し ``ReadyForArticle`` を解決する (成功主線)
- ``_persist`` で ``articles`` INSERT + ``pending_html_articles`` DELETE を
  **同 tx で一括 commit**。他 Stage (Extraction/Assessment/Embedding) と同形で
  race-loss (``save_ready`` が ``None``) は読み戻さず log + ``None`` で短絡、
...
class ArticleCompletionService:
    ...
    async def execute(self, pending_id: int) -> int | None:
        ...
        if pending is None or pending.row_meta.status != "running":
            return None
        ready = await self._resolve_ready(pending)
        if ready is None:
            return None
        return await self._persist(pending, ready)
    ...
    async def _resolve_ready(
        self, pending: PendingHtmlContext
    ) -> ReadyForArticle | None:
        """HTML 取得 → 抽出判定 → promotion で ``ReadyForArticle`` を解決する。
    ...
    async def _persist(
        ...
        他 Stage (Extraction/Assessment/Embedding) と同形。``save_ready`` が
        ``None`` を返すのは ``articles.source_url UNIQUE`` 衝突 (race-loss) の
        ただ一意のケース — articles の UNIQUE は ``source_url`` 1 つのみで、
        ``ReadyForArticle`` invariant が CHECK 違反を構造的に排除するため
        persist anomaly は構造上発生しない。よって読み戻しはせず log + ``None``
        で短絡する。pending は race / 成功どちらも DELETE する (残すと同 URL の
        再投入ループになる)。真の DB 異常は握り潰さず例外として伝播させる
        (``execute`` → taskiq task へ。失敗の見える化を優先)。成功は永続化済
        ``article_id`` を返す。
        """
