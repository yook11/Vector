...
class ArticleCompletionService:
    ...
    async def _load(self, pending_id: int) -> PendingHtmlContext | None:
        ...
        async with self._session_factory() as session:
            ...
            return await queue.find_by_id(pending_id)
    ...
    async def _persist(
        ...
        他 Stage (Extraction/Assessment/Embedding) と同形。``save`` が
        ``None`` を返すのは ``articles.source_url UNIQUE`` 衝突 (race-loss) の
        ただ一意のケース — articles の UNIQUE は ``source_url`` 1 つのみで、
        ``AnalyzableArticle`` invariant が CHECK 違反を構造的に排除するため
        ...
        async with self._session_factory() as session:
            ...
            article_id = await article_store.save(advanced)
            await pending_queue.delete_one(row_meta.id)
