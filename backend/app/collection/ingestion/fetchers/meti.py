...
Phase 3 PR 3-h-2。経済産業省の新着情報 Atom feed
(``/ml_index_release_atom.xml``) を MEXT 同型の Pattern H で扱う。

per-source 設計 (実 feed 観察ベース、SSoT: tier1-fetcher-research.md §PR 3-h-2):

- feed が **Atom 1.0** ルート (``<feed xmlns="http://www.w3.org/2005/Atom">``)、
  UTF-8 エンコード
- channel-level ``<dc:language>ja</dc:language>`` のみ言語情報を提供
- 各 ``<entry>`` は ``<title>`` / ``<link rel="alternate" type="text/html"
  href="...">`` / ``<updated>`` / ``<summary>`` の標準 Atom 構成
- ``<summary>`` は 300-400 字程度のリード文のみ (本文は detail HTML)、
  body は HTML から抽出 (Pattern H、Stage 2 の責務)
- ``<author><name /></author>`` は空 (記事 byline は HTML 側のみ)
- ``<link rel="alternate" href="URL">`` を feedparser が ``entry.link`` に、
  同 URL を ``entry.id`` にマップする (Atom の ``<id>`` 不在時の標準動作)
- robots.txt は /press/ 配下を許可、Crawl-delay 指定なし (cron 周期で自然
  rate limit)
...
- ``feedparser.parse(response.content)`` (bytes) を採用 (XML 宣言の encoding
  を feedparser に sniff させて MEXT/MIC と統一経路)
"""
...
def _extract_guid(entry: dict[str, Any]) -> str | None:
    """Atom の ``entry.id`` (``<id>`` 不在時は ``<link href>`` 流用) を返す。"""
...
class METIFetcher:
    """METI (経済産業省) 用 Pattern H Fetcher。

    PROVIDES に列挙したフィールドは feed-level / Atom 仕様で 100% 提供される
    前提:

    - ``language``: feed-level ``<dc:language>ja</dc:language>``
    - ``guid``: Atom ``entry.id`` (``<link href>`` への fallback あり)
    - ``site_name``: hardcode "METI"
    """
