...
class SpaceflightNowFetcher:
    ...
    async def fetch(self, source: NewsSource) -> AsyncIterator[FetchOutcome]:
        ...
        for entry in feed.entries:
            yield self._convert_entry(entry, source)
    ...
    def _convert_entry(
        ...
        metadata = FetchedMetadata(
            author=None,  # Spaceflight Now RSS は byline を提供しない
            tags=_extract_tags(entry),
            image_url=None,  # Spaceflight Now RSS は <media:content> を提供しない
