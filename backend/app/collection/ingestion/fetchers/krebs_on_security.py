...
に full body (3600-5800 chars) を含む、極めてクリーンな WordPress 出力 source。

per-source 設計:

- body は ``entry.content[0].value`` を**直取り**
- author は ``entry.author`` (Brian Krebs 単一だが値は出る)
- image は **``None`` 直書き** — 本ソースの RSS は ``<media:content>`` を提供しない
- language は ``feed.feed.language`` (``<channel>/<language>`` 提供あり)

旧 ``fetchers/rss/krebs_on_security.py`` (BaseRssFetcher 継承の薄スタブ) は
...
def _extract_body(entry: dict[str, Any]) -> str:
    ...
    Krebs の RSS は ``<description>`` にも本文を持つが ``<content:encoded>`` と
    重複するため、片方しか採用しない。本ソースでは content:encoded が常に full
    body かつクリーンなので summary fallback は不要。
    """
...
class KrebsOnSecurityFetcher:
    """Krebs on Security 用 RSS-only Fetcher。

    本ソースは ``<media:content>`` を提供しないため image_url は常に ``None``
    で metadata に詰める (Tier 2 として ``<wfw:commentRss>`` / ``<slash:comments>``
    を持つが本 PR では拾わない)。
    """
    ...
    async def fetch(self, source: NewsSource) -> AsyncIterator[FetchOutcome]:
        ...
        if feed.bozo and not feed.entries:
            logger.warning(
                "krebs_on_security_feed_parse_error",
                source=source.name,
    ...
    def _convert_entry(
        ...
        metadata = FetchedMetadata(
            ...
            image_url=None,  # Krebs RSS は <media:content> を提供しない
