"""NSF (米国国立科学財団) 用 Fetcher — Pattern H、RSS 2.0、UTF-8。

Phase 3 PR 3-a。NSF News feed (`/rss/rss_www_news.xml`) を Pattern H で扱う。
description は記事概要 (~170 chars + ellipsis) で本文は HTML 抽出に委譲。

per-source 設計 (実 RSS 観察ベース、SSoT: tier1-fetcher-research.md §PR 3-a):

- feed が **RSS 2.0** (UTF-8、独自 generator)
- ``<item>`` が ``<title>`` / ``<link>`` / ``<description>`` (概要+ellipsis) /
  ``<pubDate>`` RFC 822 / ``<dc:creator>`` / ``<guid isPermaLink="false">`` を提供
- per-entry の image_url / tags は **未提供** のため metadata は None / () 直書き
- ``<dc:creator>`` は事実上 "NSF" の固定値だが、metadata.author に詰めて
  将来 named author 移行に備える (PROVIDES には含めない)
- robots.txt: `*` Allow、Crawl-delay なし
- License: 17 U.S.C. §105 (Public Domain)、attribution は news_sources 行の
  ``attribution_label`` カラム ("NSF") に格納
- ``feedparser.parse(response.content)`` (bytes) を採用
"""
...
def _extract_guid(entry: dict[str, Any]) -> str | None:
    """``<guid>`` の値を返す。NSF は isPermaLink="false" だが URL を使っている。"""
...
class NSFFetcher:
    """NSF (米国国立科学財団) 用 Pattern H Fetcher。
