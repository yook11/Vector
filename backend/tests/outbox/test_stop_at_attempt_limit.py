...
async def test_stop_selects_and_returns_events_by_occurrence_time(db_session):
    ...
    result = await OutboxDeliveryRepository(
        db_session
    ).stop_deliveries_at_attempt_limit(event_type=EVENT_TYPE, limit=2)
    ids = result
...
async def test_stop_event_type_is_exact_and_old_events_are_included(db_session):
    ...
    assert (
        await repository.stop_deliveries_at_attempt_limit(event_type=f" {EVENT_TYPE} ")
        == []
    )
    result = await repository.stop_deliveries_at_attempt_limit(event_type=EVENT_TYPE)
...
async def test_stop_scope_validation_precedes_database_access(fields, exception):
...
@pytest.mark.parametrize("limit,expected_count", [(None, 100), (2, 2), (12, 12)])
async def test_stop_limit_leaves_excess_events_unchanged(
    ...
    result = await OutboxDeliveryRepository(
        db_session
    ).stop_deliveries_at_attempt_limit(event_type=EVENT_TYPE, **kwargs)
    stopped_ids = set(result)
    assert len(result) == len(stopped_ids) == expected_count
...
async def test_next_stop_processes_events_left_unstopped(db_session):
    ...
    assert len(remaining) == 2
    second = await repository.stop_deliveries_at_attempt_limit(event_type=EVENT_TYPE)
    assert len(second) == 2
