...
from app.outbox.sqs_error_mapping import SqsBatchEntryError
...
def test_all_invalid_events_do_not_construct_an_empty_batch(envelope, monkeypatch):
    ...
    monkeypatch.setattr("app.outbox.sqs_publisher.SqsMessageBatch", make_batch)
    sender, _, factory = _sender()
...
def test_whole_request_classifier_failure_keeps_both_types(envelope, monkeypatch):
    ...
    monkeypatch.setattr(
        "app.outbox.sqs_error_mapping._publish_error_from_sqs_exception",
...
def test_entry_classifier_failure_preserves_other_results(envelope, monkeypatch):
    ...
    from app.outbox.sqs_error_mapping import _service_error_from_sqs_code
    ...
    monkeypatch.setattr(
        "app.outbox.sqs_error_mapping._service_error_from_sqs_code", classify
    )
    ...
    assert error.original_exception_type == (
        "app.outbox.sqs_error_mapping.SqsBatchEntryError"
    )
    assert error.classification_exception_type == "builtins.ValueError"
...
def test_oversized_event_stops_without_configuration_alarm(envelope):
    from app.outbox.publish_failure_recording import (
...
def test_checksum_calculation_failure_is_local_before_send(
    ...
    monkeypatch.setattr("app.outbox.sqs_message.md5", hash_call)
