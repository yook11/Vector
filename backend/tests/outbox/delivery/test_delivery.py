...
async def test_schedule_retry_records_database_time_plus_delay_and_releases_lease(
    ...
    updated = await OutboxDeliveryRepository(db_session).schedule_retry(
        event_id=event_id, lease_token=token, retry_delay=RetryDelay(delay)
    )
...
@pytest.mark.parametrize("reason", list(NonRetryableReason))
...
@pytest.mark.parametrize(
    ...
    [
        ("mark_published", {}),
        ("schedule_retry", {"retry_delay": RetryDelay(timedelta(minutes=5))}),
        ("stop_delivery", {"reason": NonRetryableReason.RETRY_EXHAUSTED}),
    ],
)
@pytest.mark.parametrize(
