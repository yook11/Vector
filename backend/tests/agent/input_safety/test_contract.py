...
def test_agent_output_accepts_allow_and_exactly_five_application_block_reasons() -> (
    None
):
    agent_output_type = _attribute("InputSafetyAgentOutput")
    agent_reason_type = _attribute("InputSafetyAgentBlockReason")

    allow = agent_output_type.model_validate(  # type: ignore[attr-defined]
        {"input_safety_result": "allow", "block_reason": None}
    )
    reason_values = [reason.value for reason in agent_reason_type]  # type: ignore[union-attr]
    blocked = [
...
def test_application_result_keeps_provider_filter_out_of_serialized_properties() -> (
    None
):
    ...
    provider_block = result_type.model_validate(  # type: ignore[attr-defined]
        {
            "input_safety_result": "block",
            "block_reason": "provider_safety_filter",
        }
    ...
    assert provider_block.is_blocked is True
    assert "is_blocked" not in provider_block.model_dump()
    assert "provider_safety_filter" in {
        member.value
        for member in _attribute("InputSafetyBlockReason")  # type: ignore[union-attr]
    }
