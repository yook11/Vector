...
def test_query_typed_input_is_immutable() -> None:
    _assert_frozen_slots_dataclass(ExternalQueryGenerationInput)
    assert [field.name for field in fields(ExternalQueryGenerationInput)] == [
        "task",
        "as_of",
        "target_time_window",
    ]
...
def test_query_draft_filters_non_strings_before_workflow_normalization() -> None:
    draft = ExternalQueryDraft.model_validate(
        {"queries": ["  NVIDIA  ", 123, "", None, "NVIDIA"]}
    )

    assert draft.queries == ["  NVIDIA  ", "", "NVIDIA"]
    with pytest.raises(ValidationError):
        ExternalQueryDraft.model_validate({"queries": "not a list"})
...
def test_query_prompt_renders_typed_window_or_none_deterministically() -> None:
    renderer = EXTERNAL_QUERY_AGENT.prompt.input_renderer
