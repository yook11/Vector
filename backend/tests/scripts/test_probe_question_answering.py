...
def test_search_probe_passes_actual_internal_and_external_dependencies_to_phases() -> (
    ...
    embedder = _keyword_value(internal_search_call, "embedder")
    repository = _keyword_value(internal_search_call, "article_search_repository")
    ...
    # internal searchはSSEのprogress reporterを知らない (段2契約)。
    assert "events" not in {keyword.arg for keyword in internal_search_call.keywords}
    ...
    service_targets = {
        ...
        for assignment in ast.walk(search)
        if isinstance(assignment, ast.Assign)
        and assignment.value is internal_search_call
        for target in assignment.targets
        if isinstance(target, ast.Name)
    }
    events_targets = {
    ...
    assert isinstance(external_search_scope, ast.Name)
    assert external_search_scope.id == "activate_external_search"
    reviewer = _keyword_value(phase, "reviewer")
...
def test_direct_probe_keeps_dependencies_unreachable_and_uses_plan_summary() -> None:
    ...
    assert _call_name(external_search_scope) == "_UnreachableExternalSearchScope"
