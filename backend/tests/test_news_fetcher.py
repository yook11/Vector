...
async def test_fetch_saves_new_articles(
    ...
    assert all(a.source_id == sample_source.id for a in articles)
...
async def test_fetch_handles_304_not_modified(
    db_session: AsyncSession, sample_source: NewsSource, mock_client: AsyncMock
) -> None:
    ...
    assert result.source_results[0].not_modified is True
