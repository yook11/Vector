"""ReadyForExtraction (Stage 3 precondition 型) のドメインユニットテスト。

PR3 案 3 化: ``try_advance_from`` は Repository の ``try_load_for_extraction``
への thin delegate になったため、本ファイルでは:

- ``try_advance_from`` が Protocol の ``try_load_for_extraction`` をそのまま
...
def _ready_repo_returning(ready: ReadyForExtraction | None) -> AsyncMock:
    """``ExtractionPreconditionProtocol`` の thin mock を返す。"""
    repo = AsyncMock()
    repo.try_load_for_extraction = AsyncMock(return_value=ready)
    return repo


# ---------------------------------------------------------------------------
# try_advance_from — thin delegate (Repository.try_load_for_extraction)
# ---------------------------------------------------------------------------
...
class TestTryAdvanceFromThinDelegate:
    ...
    async def test_returns_ready_when_repo_returns_ready(self) -> None:
        ...
        ready = await ReadyForExtraction.try_advance_from(article_id=42, repo=repo)

        assert ready is expected
        repo.try_load_for_extraction.assert_awaited_once_with(42)
    ...
    async def test_returns_none_when_repo_returns_none(self) -> None:
        ...
        ready = await ReadyForExtraction.try_advance_from(article_id=99, repo=repo)

        assert ready is None
        repo.try_load_for_extraction.assert_awaited_once_with(99)
...
# ---------------------------------------------------------------------------
# ExtractionTrigger — 軽量 ID キャリア (kiq message 用)
# ---------------------------------------------------------------------------
...
class TestExtractionTrigger:
    def test_carries_article_id_only(self) -> None:
        ...
        trigger = ExtractionTrigger(article_id=42)
        assert trigger.article_id == 42

    def test_rejects_non_positive_article_id(self) -> None:
        """article_id <= 0 は Field(gt=0) が拒否する。"""
        with pytest.raises(ValidationError):
            ExtractionTrigger(article_id=0)
        with pytest.raises(ValidationError):
            ExtractionTrigger(article_id=-1)

    def test_is_frozen(self) -> None:
        """frozen=True のため field 書き換えは ValidationError。"""
        trigger = ExtractionTrigger(article_id=1)
        with pytest.raises(ValidationError):
