"""新 EmbeddingService (app.analysis.embedding.service) の DB 統合テスト。

Phase 3 の並行追加期間用テストで、Phase 4 で本テストの内容は
``test_embedding_service.py`` に統合され、本ファイルは削除される予定。
Outcome tagged union (``EmbeddedOutcome`` / ``AlreadyEmbeddedOutcome`` /
``SkippedOutcome``) と Service フローの 6 経路を検証する。
"""
