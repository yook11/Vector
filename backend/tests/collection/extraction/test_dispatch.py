...
- ``dispatch_html_fetch_jobs``:
  ...
  - ``extract_html_body.kiq`` が claim 済 pending_id 列で正確に呼ばれる
...
async def test_dispatches_only_open_and_ready_pending(
    ...
    kiq_mock = AsyncMock()
    monkeypatch.setattr("app.collection.tasks.extract_html_body.kiq", kiq_mock)
...
async def test_increments_attempt_count_and_sets_lease(
    ...
    monkeypatch.setattr("app.collection.tasks.extract_html_body.kiq", AsyncMock())
