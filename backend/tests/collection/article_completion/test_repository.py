...
class _StubArticleSource:
    ...
    repository は ``completion_profile`` のみ参照する。``collect`` は本テストで
...
def _register_sample_source_in_registry(
    ...
    """``sample_source`` を使う test で SOURCES に ``DEFAULT_PROFILE`` を登録する。
    ...
    と整合する形 (DEFAULT_PROFILE) を default として SOURCES に挿入し、test が
...
async def test_try_load_resolves_profile_from_source_name_registry(
    ...
    ``HTML_TITLE_PROFILE`` が ``DEFAULT_PROFILE`` と区別される寄り辺を作る)。
    """
    monkeypatch.setitem(
        SOURCES,
        sample_source.name,
        _StubArticleSource(
            name=sample_source.name, completion_profile=HTML_TITLE_PROFILE
        ),
    )
    pending_id = await _enqueue(
        db_session,
    ...
    assert ready.profile is HTML_TITLE_PROFILE
...
async def test_try_load_raises_keyerror_for_unregistered_source(
    ...
    ``DEFAULT_PROFILE`` fallback (旧 ``SOURCES.get(name) or DEFAULT_PROFILE``)
