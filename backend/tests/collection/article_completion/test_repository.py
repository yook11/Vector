...
async def test_try_load_for_completion_returns_claimed_target(
    ...
    assert target.incomplete_article.title == "Find Me"
    assert target.incomplete_article.source_url == url
...
async def test_sweep_expired_leases_reopens_dead_lease(
    ...
    swept = await ArticleCompletionRepository(db_session).sweep_expired_leases(now=now)
    await db_session.commit()
...
async def test_close_claimed_closes_pending(
    ...
    updated = await ArticleCompletionRepository(db_session).close_claimed(
        target, now=now
    )
    await db_session.commit()
...
async def test_schedule_retry_reopens_with_future_ready_at(
    ...
    updated = await ArticleCompletionRepository(db_session).schedule_retry(
        target, ready_at=next_at, now=now
    )
    await db_session.commit()
