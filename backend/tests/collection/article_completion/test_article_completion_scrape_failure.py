...
def test_terminal_fetch_error_maps_to_terminal_with_code(
    ...
    """terminal (retryable=False) は ``ScrapeTerminal(reason_code=cls.CODE)``。"""
...
# ``ScrapeContentFailure`` union の各 variant を最小 kwargs で構築する表。union に
# 新 variant を足して登録し忘れると下の coverage テストが落ちる。
_CONTENT_CONSTRUCT: dict[type, dict[str, object]] = {
    ScrapeNotHtml: {"content_type": "application/pdf"},
    ScrapeParserGaveUp: {},
    ScrapeParseCrashed: {"error_class": "ValueError", "error_message": "x"},
    ScrapeContentQualityTooLow: {
        "body_length": 0,
        "title_present": False,
        "body_sample": None,
...
def test_content_construct_table_covers_union() -> None:
    assert set(_CONTENT_CONSTRUCT) == set(typing.get_args(ScrapeContentFailure))


@pytest.mark.parametrize(
    "cls", list(_CONTENT_CONSTRUCT), ids=[c.__name__ for c in _CONTENT_CONSTRUCT]
)
def test_every_content_failure_maps_to_scrape_terminal(cls: type) -> None:
    """union の全 content variant が ``scrape_*`` terminal に写る。"""
    result = classify_scrape_failure(cls(**_CONTENT_CONSTRUCT[cls]))  # type: ignore[arg-type]
    assert isinstance(result, ScrapeTerminal)
    assert result.reason_code.startswith("scrape_")


def test_unmapped_content_failure_raises() -> None:
    """union 外の content 値は totality guard (``case _``) で早期に露見する。"""

    class _UnmappedContentFailure:
        reason = "unmapped_test_only"

    with pytest.raises(AssertionError):
        classify_scrape_failure(_UnmappedContentFailure())  # type: ignore[arg-type]
