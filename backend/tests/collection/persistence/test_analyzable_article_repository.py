"""``AnalyzableArticleRepository`` の統合テスト (実 Postgres)。
...
async def test_save_persists_ready_article(
    ...
    repo = AnalyzableArticleRepository(db_session)
    ready = _ready(sample_source.id, "https://example.com/article/save-ready")

    article_id = await repo.save(ready)
    assert isinstance(article_id, int)
    assert article_id > 0
    await db_session.commit()

    orm = await db_session.get(AnalyzableArticleRecord, article_id)
...
async def test_save_returns_none_on_duplicate_source_url(
    ...
    first = await repo.save(_ready(sample_source.id, url))
    await db_session.commit()
    assert first is not None

    second = await repo.save(_ready(sample_source.id, url))
