...
- 入力と期待結果はテスト本文から読める形にする。通し番号や真偽値のフラグでシナリオを隠さない。parametrize は、各行を見ても何を検証しているか分かり、かつ同じことを検証しているときにだけ使う。同じ例外や同じ assert だけでは、同じ検証とはしない。
- サニタイズのテストは保護対象・規則ごとに分け、APIプロバイダー、Cookie、Authorization、接続文字列を「置換する」という共通点だけでparametrizeにまとめない。
