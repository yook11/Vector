"""Stage 3 ACL — ``map_provider_to_extraction`` の dispatch / 網羅性テスト。
...
from app.analysis.extraction.errors import (
    EXTRACTION_RECOVERABLE_PROVIDER_ERRORS,
    EXTRACTION_TERMINAL_DROP_PROVIDER_ERRORS,
    EXTRACTION_TERMINAL_KEEP_PROVIDER_ERRORS,
...
class TestTupleContents:
    ...
    def test_recoverable_tuple_contains_expected_types(
        ...
        assert exc_type in EXTRACTION_RECOVERABLE_PROVIDER_ERRORS
    ...
    def test_terminal_keep_tuple_contains_expected_types(
        ...
        assert exc_type in EXTRACTION_TERMINAL_KEEP_PROVIDER_ERRORS
    ...
    def test_terminal_drop_tuple_contains_expected_types(
        ...
        assert exc_type in EXTRACTION_TERMINAL_DROP_PROVIDER_ERRORS

    def test_three_tuples_cover_all_expected_provider_error_types(self) -> None:
        # 網羅性: 期待 9 種すべてが 3 tuple のいずれかに登録されている。
        union = (
            frozenset(EXTRACTION_RECOVERABLE_PROVIDER_ERRORS)
            | frozenset(EXTRACTION_TERMINAL_KEEP_PROVIDER_ERRORS)
            | frozenset(EXTRACTION_TERMINAL_DROP_PROVIDER_ERRORS)
        )
    ...
    def test_three_tuples_are_mutually_exclusive(self) -> None:
        ...
        recoverable = frozenset(EXTRACTION_RECOVERABLE_PROVIDER_ERRORS)
        terminal_keep = frozenset(EXTRACTION_TERMINAL_KEEP_PROVIDER_ERRORS)
        terminal_drop = frozenset(EXTRACTION_TERMINAL_DROP_PROVIDER_ERRORS)
    ...
    def test_recoverable_tuple_size(self) -> None:
        assert len(EXTRACTION_RECOVERABLE_PROVIDER_ERRORS) == 4

    def test_terminal_keep_tuple_size(self) -> None:
        assert len(EXTRACTION_TERMINAL_KEEP_PROVIDER_ERRORS) == 3

    def test_terminal_drop_tuple_size(self) -> None:
        assert len(EXTRACTION_TERMINAL_DROP_PROVIDER_ERRORS) == 2


class TestMapProviderToExtractionRecoverable:
    """Recoverable 系 4 種が ``ExtractionRecoverableError`` に詰め替えられる。"""
    ...
    def test_dispatches_to_recoverable_marker(
        ...
        result = map_provider_to_extraction(original)

        assert isinstance(result, ExtractionRecoverableError)
...
class TestMapProviderToExtractionTerminalKeep:
    """TerminalKeep 系 3 種が ``ExtractionTerminalKeepError`` に詰め替えられる。"""
    ...
    def test_dispatches_to_terminal_keep_marker(
        ...
        assert isinstance(result, ExtractionTerminalKeepError)
...
class TestMapProviderToExtractionTerminalDrop:
    """TerminalDrop 系 2 種が ``ExtractionTerminalDropError`` に詰め替えられる。"""
    ...
    def test_dispatches_to_terminal_drop_marker(
        ...
        assert isinstance(result, ExtractionTerminalDropError)
...
class TestMapProviderToExtractionUnregistered:
    ...
    def test_unregistered_subclass_raises_type_error(self) -> None:
        ...
        with pytest.raises(TypeError, match="unmapped provider error"):
            map_provider_to_extraction(unregistered)
    ...
    def test_bare_provider_error_base_raises_type_error(self) -> None:
        ...
        with pytest.raises(TypeError, match="unmapped provider error"):
            map_provider_to_extraction(bare)
