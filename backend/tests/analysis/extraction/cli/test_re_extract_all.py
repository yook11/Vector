"""re_extract_all CLI のテスト (Phase 1B α-1)。
...
def _summary_from_stdout(captured: str) -> dict:
    ...
    for line in reversed(captured.splitlines()):
        line = line.strip()
        if line.startswith("{"):
            return json.loads(line)["re_extract_summary"]
...
async def test_run_default_dry_run_processes_3_articles_at_most(
    ...
    args = build_parser().parse_args([])
    code = await run(args, service, extractor, session_factory)
...
async def test_run_returns_3_when_any_failed(
    ...
    async def _fake_execute(self, article_ids, extractor, *, dry_run):  # noqa: ARG001
    ...
    monkeypatch.setattr(ReExtractionService, "execute", _fake_execute)
    ...
    monkeypatch.setattr(
        "app.analysis.extraction.cli.re_extract_all._select_article_ids",
        _fake_select,
    )
