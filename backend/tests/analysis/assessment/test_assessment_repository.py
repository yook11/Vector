...
in-scope / out-of-scope 永続化責務を 1 class に統合した repository を検証する。
...
- ``save_out_of_scope`` 既存 row 存在時 (race lost) には ``None`` を返し、勝者 row
  はそのまま残る
- ``save_in_scope`` 成功時、戻り値 ``(id, category_id)`` が DB 上の値と一致する
- ``save_in_scope`` race lost で ``None`` を返す
- ``save_in_scope`` で AI が未登録 slug を返したら ``AssessmentCategoryMissingError``
- ``exists_in_scope`` / ``exists_out_of_scope`` が row 有無で正しく分岐する
"""
...
async def test_save_in_scope_persists_snapshot_fields(
    ...
    """``save_in_scope`` 成功時、戻り値 ``(id, category_id)`` と DB row が一致する。"""
    extraction = await _make_extraction(db_session, sample_source)
    repo = AssessmentRepository(db_session)
    ...
    saved_id, saved_category_id = saved
    assert saved_id > 0
    ...
    assert row.id == saved_id
    assert row.category_id == saved_category_id
    assert row.translated_title == extraction.translated_title
    assert row.summary == extraction.summary
...
# ---------------------------------------------------------------------------
# exists_in_scope / exists_out_of_scope (try_advance_from precondition)
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_exists_in_scope_returns_true_when_row_present(
    db_session: AsyncSession,
    sample_source: NewsSource,
    sample_categories: list[Category],
) -> None:
    ...
    repo = AssessmentRepository(db_session)
    assert await repo.exists_in_scope(extraction.id) is True
    assert await repo.exists_out_of_scope(extraction.id) is False


@pytest.mark.asyncio
async def test_exists_out_of_scope_returns_true_when_row_present(
    db_session: AsyncSession,
    sample_source: NewsSource,
) -> None:
    extraction = await _make_extraction(db_session, sample_source)
    ...
    repo = AssessmentRepository(db_session)
    assert await repo.exists_out_of_scope(extraction.id) is True
    assert await repo.exists_in_scope(extraction.id) is False
