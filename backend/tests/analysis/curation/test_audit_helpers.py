"""``base_curation_payload_fields`` の振る舞いテスト (PR1-a で helper 縮退)。
...
def test_returns_4_fields_in_canonical_keys() -> None:
    ...
    fields = base_curation_payload_fields(
...
def test_helper_does_not_include_ai_model_or_prompt_version() -> None:
    ...
    """
    fields = base_curation_payload_fields(original_content="x")
...
def test_input_content_length_is_raw_length() -> None:
    ...
    fields = base_curation_payload_fields(original_content=raw)
...
def test_source_name_string_is_preserved() -> None:
    fields = base_curation_payload_fields(original_content="x", source_name="MIT News")
...
def test_input_content_hash_changes_when_content_changes_within_truncation_window() -> (
    ...
    f1 = base_curation_payload_fields(original_content="alpha")
    f2 = base_curation_payload_fields(original_content="beta")
...
def test_input_content_hash_unchanged_when_change_is_after_truncation_window() -> None:
    ...
    f1 = base_curation_payload_fields(original_content=head + "short")
    f2 = base_curation_payload_fields(original_content=head + "much-longer-tail")
