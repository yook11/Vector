...
class TestCreateAppEngine:
    ...
    def test_caller_can_override_hide_parameters(self) -> None:
        engine = _create_engine(f"{_NEON}?sslmode=require", hide_parameters=False)
    ...
    def test_caller_passed_ssl_connect_arg_raises(self) -> None:
        ...
        with pytest.raises(ValueError, match="connect_args"):
            _create_engine(f"{_NEON}?sslmode=require", connect_args={"ssl": ctx})
    ...
    def test_non_ssl_connect_args_are_merged(
        ...
        _create_engine(f"{_NEON}?sslmode=require", connect_args={"command_timeout": 30})
    ...
    def test_no_application_name_leaves_server_settings_absent(
        ...
        monkeypatch.setattr(db_engine, "create_async_engine", _spy)
        _create_engine(f"{_NEON}?sslmode=require")
...
class TestPasswordProvider:
    ...
    def _captured_connect_args(
        ...
        monkeypatch.setattr(db_engine, "create_async_engine", _spy)
        _create_engine(f"{_NEON}?sslmode=require", **factory_kwargs)
    ...
    def test_caller_passed_password_raises(self) -> None:
        ...
        with pytest.raises(ValueError, match="connect_args"):
            _create_engine(f"{_NEON}?sslmode=require", connect_args={"password": "x"})
...
class TestEngineResilienceDefaults:
    ...
    def test_caller_override_wins(self) -> None:
        engine = _create_engine(f"{_NEON}?sslmode=require", pool_recycle=60)
