"""``app/insights/briefing/domain/week.py`` 純関数のテスト。

``latest_completed_week_start`` は briefing dispatcher / CLI から参照される
週境界算出。Snapshot BC とは独立に持つ (BC ごとの責務分離)。
"""
...
from app.insights.briefing.domain.week import latest_completed_week_start
