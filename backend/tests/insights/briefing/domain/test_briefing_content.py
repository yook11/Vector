...
class TestBriefingStory:
    def test_min_length_constraints(self) -> None:
        with pytest.raises(ValidationError):
            BriefingStory(title="", analysis="x", article_ids=[1])
        with pytest.raises(ValidationError):
            BriefingStory(title="x", analysis="", article_ids=[1])

    def test_article_ids_min_length_1(self) -> None:
        with pytest.raises(ValidationError):
            BriefingStory(title="x", analysis="x", article_ids=[])


class TestWeeklyBriefingContent:
    def test_empty_stories_rejected(self) -> None:
        with pytest.raises(ValidationError):
            WeeklyBriefingContent(headline="x", stories=[])
    ...
    def test_subset_check_passes_when_no_context(self) -> None:
        ...
        content = WeeklyBriefingContent(
            headline="今週は AI",
            stories=[BriefingStory(title="t", analysis="a", article_ids=[42])],
        )
    ...
    def test_subset_check_passes_when_all_ids_in_input(self) -> None:
        data = {
            "headline": "今週は AI",
            "stories": [{"title": "t", "analysis": "a", "article_ids": [1, 2]}],
        }
    ...
    def test_subset_check_rejects_unknown_id(self) -> None:
        ...
        data = {
            "headline": "今週は AI",
            "stories": [{"title": "t", "analysis": "a", "article_ids": [1, 999]}],
        }
