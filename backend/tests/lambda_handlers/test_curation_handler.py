"""Curation入口の入力の受け渡し・失敗範囲・処理順を確認する。"""
...
def test_each_payload_is_passed_to_the_shared_consumer(wiring):
    ...
    assert wiring.consumer.consume.await_args_list == [
        call(AnalyzableArticleCreated(analyzable_article_id=101)),
        call(AnalyzableArticleCreated(analyzable_article_id=202)),
    ]
...
async def test_messages_finish_sequentially_in_input_order(wiring):
    ...
    await module._run_curation({"Records": messages}, wiring.settings)
...
def test_invalid_message_does_not_prevent_following_message(wiring, invalid_message):
    ...
    wiring.consumer.consume.assert_awaited_once_with(
        AnalyzableArticleCreated(analyzable_article_id=101)
    )
...
@pytest.mark.parametrize(
    "completion,reason",
    [
        (CurationCompletion(CurationCompletionKind.SIGNAL, 901), "signal"),
        (CurationCompletion(CurationCompletionKind.NOISE), "noise"),
        (
            CurationCompletion(CurationCompletionKind.ALREADY_CURATED),
            "already_curated",
        ),
...
def test_unexpected_parser_failure_does_not_stop_batch(wiring, monkeypatch):
    ...
    body = valid_body()
    parsed = module.parse_analyzable_article_created_event(body)
    monkeypatch.setattr(
        module,
