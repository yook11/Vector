"""AssessmentのJSON解析と、共有イベント契約への接続を確認する。"""
...
def test_invalid_json_is_rejected_without_retaining_body(body):
    ...
    with pytest.raises(AssessmentEventInvalidError) as caught:
        parse_curated_signal_event(body)
    error = caught.value
    assert error.CODE == "assessment_event_invalid"
    assert error.reason is AssessmentEventInvalidReason.INVALID_JSON
    assert error.issues == ()
...
def test_deep_json_is_reported_as_invalid_json():
    ...
    with pytest.raises(AssessmentEventInvalidError) as caught:
        parse_curated_signal_event(body)
    assert caught.value.reason is AssessmentEventInvalidReason.INVALID_JSON
...
def test_valid_json_with_wrong_root_uses_event_contract():
    ...
    with pytest.raises(AssessmentEventInvalidError) as caught:
        parse_curated_signal_event("[]")
    assert caught.value.reason is AssessmentEventInvalidReason.INVALID_ENVELOPE
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"schema_version": 2},
        {"payload": {"curation_id": "private-id", "analyzable_article_id": 3}},
    ],
)
def test_shared_rejection_preserves_reason_and_details_without_exception_chain(
    ...
    data.update(changes)
    with pytest.raises(CuratedEventValidationError) as shared:
