"""backfillロールの権限migrationの往復と、既存の権限・データの維持を確認する。"""
...
REVISION = "z26_grant_backfill"
PREDECESSOR = "z25_grant_article_analysis"
...
async def _backfill_grants(database):
...
async def test_backfill_grants_round_trip(predecessor_database):
    """付与した権限だけがdowngradeで消え、再upgradeで同じ権限へ戻る。"""
    database = predecessor_database
    assert await _backfill_grants(database) == NO_GRANTS

    await migrate(database, command.upgrade, REVISION)
    granted = await _backfill_grants(database)
    tables, columns, sequences, schema_grants, connect_granted = granted
    assert ("public", "analyzable_articles", "DELETE") in tables
    assert columns[("public", "article_curations", "UPDATE")] == {"id"}
    assert sequences
    ...
    await migrate(database, command.upgrade, REVISION)
    assert await _backfill_grants(database) == granted
...
async def test_backfill_migration_preserves_other_role_permissions(
...
async def _backfill_rows(connection, article_id, incomplete_id):
    return (
        ...
        await connection.fetch(
            "SELECT * FROM incomplete_articles WHERE id=$1", incomplete_id
        ),
    )


async def test_backfill_migration_preserves_existing_rows(predecessor_database):
    """権限を往復しても既存の記事・curation・分析結果・未完成行を変更しない。"""
    database = predecessor_database
    async with database.connect("vector") as connection:
        ...
        incomplete_id = await connection.fetchval(
            "INSERT INTO incomplete_articles "
            "(url, source_id, source_name, status, observed_article, ready_at) "
            "SELECT 'https://example.com/incomplete', id, name, 'open', "
            "'{}'::jsonb, now() FROM news_sources ORDER BY id LIMIT 1 RETURNING id"
        )
        before = await _backfill_rows(connection, article_id, incomplete_id)
    assert [len(rows) for rows in before] == [1, 1]

    for operation, revision in ROUND_TRIP:
        await migrate(database, operation, revision)
        async with database.connect("vector") as connection:
            assert await _backfill_rows(connection, article_id, incomplete_id) == before


async def test_missing_role_fails_before_granting(predecessor_database, monkeypatch):
    """クラスタ共通ロールを変更せず、存在しないロールで事前条件を検証する。"""
    path = ROOT / "backend/alembic/versions/z26_grant_backfill.py"
    spec = importlib.util.spec_from_file_location("backfill_migration", path)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    monkeypatch.setattr(module, "ROLE_NAME", "vector_backfill_missing")
    ...
    try:
        with pytest.raises(RuntimeError, match="Create vector_backfill"):
            async with engine.begin() as connection:
                await connection.run_sync(run)
    finally:
        await engine.dispose()
    assert await _backfill_grants(predecessor_database) == NO_GRANTS
