...
async def test_read_session_is_returned_while_waiting_for_ai(
    system_database, embedding_runtime, analysis_engines, gated_ai_responses
):
    """AI応答の待機中は、貸出接続も実DBのトランザクションも残らない。"""
    target = await seed_article(system_database, "https://example.com/ai-wait")
    gate = gated_ai_responses([0.2] * EMBEDDING_DIMENSION)
    invocation = asyncio.create_task(invoke_event(target))
    try:
        await gate.wait_requested("対象記事")
