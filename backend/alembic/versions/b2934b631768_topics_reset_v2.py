...
v1 トピック分類 (#110, 739f1cf06fae) の稼働後に発覚した 2 つの欠陥
- 表記揺れ重複（`ai agents` vs `ai-agents` など）
- AI 適用先をトピック化する細分化（`ai-driven business process automation` 等）
を解消するため、全 analyses / rejections / topics を再度 wipe し、シード 25 件を再投入する。

TopicName VO を「ハイフン→空白」正規化 + 最大 3 語 + 冠詞/前置詞 reject に差し替えた
変更と組で適用する。SEED は v1 と同一内容だが、`post-quantum cryptography` のみ
正規化後の canonical 形である `post quantum cryptography` に書き換える
（VO 経由の find_or_create_topic がヒットするように）。

Revision ID: b2934b631768
Revises: 6640e41ea840
Create Date: 2026-04-23 22:57:45.694511
...
revision: str = "b2934b631768"
down_revision: str | None = "6640e41ea840"
