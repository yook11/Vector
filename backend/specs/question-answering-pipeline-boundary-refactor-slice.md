# Question Answering Pipeline 境界整理仕様
...
- **Pipeline**: 1 つの回答経路内の生成・検証・retry・fallback・観測。
...
3. `direct.py` は contract と実装を 1 module に持ち、`synthesis.py` はさらに
   contract、pipeline、決定的補正、citation 検証、audit 呼び出しまで持つ。
...
`EvidenceAnswerPipeline` は evidence 自体の品質管理サービスではない。
...
`EvidenceAnswerPipeline` が保証するのは、与えられた evidence を入力として
...
best-effort であり、この pipeline の決定的保証には含めない。

### Contract と Pipeline の境界
...
`pipeline.py` は「その保証をどう実現するか」を所有する。
...
専用 `validation.py` を作らず `direct_answer/pipeline.py` 内に残す。
...
| `DirectAnswerService` | `DirectAnswerPipeline` | direct 回答経路 |
| `AnswerSynthesisService` | `EvidenceAnswerPipeline` | evidence 回答経路 |
...
`Orchestrator` / `Pipeline` / `Generator` を使う。
...
具象 `DirectAnswerPipeline` / `EvidenceAnswerPipeline` や Gemini adapter を
...
生成か evidence synthesis かは Pipeline 内へ閉じる。
...
- typed error は provider adapter、validation、pipeline、上位 catch 面で共有する
...
`direct_answer/pipeline.py` は現在の `DirectAnswerService` の実装責務を移す。
...
`evidence_answer/pipeline.py` は現在の `AnswerSynthesisService` のうち、
オーケストレーション責務を移す。
...
Pipeline の公開 method は両方 `answer(...)`、generator の公開 method は両方
`generate(...)` とする。
...
- retry / fallback / audit は決めない。補完不能時は contract の typed error
  または Pydantic `ValidationError` を送出し、Pipeline が扱う。
...
composition.py
  -> Orchestrator + Pipeline + Generator の具象型を組み立てる
```

詳細規則:

- orchestration は Pipeline / Gemini の具象型を import しない。
- Pipeline は Gemini adapter を import せず、注入された Generator Protocol
  だけに依存する。
- AI adapter は orchestration を import しない。
- contract は pipeline / validation / audit / metrics / provider を import しない。
...
- composition 以外で具象 Pipeline を要求するコードを新たに作らない。
...
- answer 本文 citation marker が cited source の正本であり、存在しない ref を
  final result に通さない。
- evidence 0 件でも evidence pipeline を呼び、valid な insufficient draft
  または fallback を返す。
...
- 循環 import を service locator や runtime import の追加で隠さず、依存方向を
  直して解消する。
- 重複排除だけを目的に direct / evidence 共通 base Pipeline を作らない。
...
- direct / evidence 共通 Pipeline 基底 class の導入。
...
3. evidence の決定的補正・citation 検証を `validation.py` へ移し、既存
   synthesis test を validation / pipeline test に責務別に分ける。
4. `DirectAnswerPipeline` と `EvidenceAnswerPipeline` を `pipeline.py` へ移す。
...
backend/app/agent/answering/direct_answer/pipeline.py                (new)
...
backend/app/agent/answering/evidence_answer/pipeline.py              (new)
...
1. `composition.py` が Orchestrator、Pipeline、Generator を組み立てる。
...
- `Orchestrator → Pipeline → Generator` の語彙が class 名と package 構造で
  一致している。
- direct / evidence の contract と pipeline が対称な位置にある。
- evidence の draft finalization / citation validation が Pipeline から分離され、
  provider・retry・audit に依存していない。
- Orchestrator は具象 Pipeline / Gemini adapter に依存していない。
