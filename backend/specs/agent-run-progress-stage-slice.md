...
> `agent-run-progress-stage-drop-slice.md` は先にアプリ・APIの参照を外し、全ECS taskの
> rollout完了後に`z17_drop_progress_stage`で列とCHECKを落とす。表示正本は Redis Stream
> （SSE）であり、本文中の「報告ごとに DB を UPDATE する」記述は無効である。
...
- **acquire_for_execution** が queued/running → running の条件付き UPDATE を持つ
  （`runs/repository.py`）。再配送復旧時も同経路で running に戻る。
...
3. **書き込みは worker 側 reporter が所有する**: `app/agent/runs/progress.py`
...
backend/app/agent/runs/progress.py      (新規: AgentRunProgressWriter — 条件付き UPDATE、best-effort)
backend/app/agent/runs/projection.py    (progressStage 写像追加)
