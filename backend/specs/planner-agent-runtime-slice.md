...
実装状況: In Progress（Agent / Prompt宣言まで完了）
...
文字列化せず、`loc`の文字列 / 整数、`type`、安全と確認した`ctx` keyだけを明示的に選ぶ。生の`input`、
provider response、自由文`msg`、`url`、任意の`ctx`を含めない。invalid JSON / non-objectではraw responseを
detail化せず、固定の安全なrepair hintだけを使う。
...
render済みinput、requirements、`previous_error`、provider response、draft、completed plan、prompt version、
final answerをphase / attempt spanへ記録しない。
...
Prompt固有metadataである。Runtimeの戻り値やprovider requestへ含めず、trace attributeにも記録しない。
...
- `agent_provider_call`の`gen_ai.operation.name="generate_content"`、
  `gen_ai.provider.name="gcp.gemini"`、`gen_ai.request.model`を検証する。
- `agent_phase`の最終独自attribute keyが`phase` / `agent_name`、`agent_provider_call`が`agent_name` /
  `attempt_number` / 条件付き`result`だけであることを検証する。Logfire / OpenTelemetry内部attributeは
  アプリケーション独自attributeの比較対象外とする。
- attribute、exceptionを含むevent、status descriptionを含むspan全面を検査し、入力由来sentinel、
  model-visible text、prompt version、childへの`run_id`複写がない。
...
- Prompt versionが`AgentPrompt`のmetadataとしてだけ存在し、Runtime戻り値やprovider requestへ混入しない。
