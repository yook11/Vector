...
Status: Draft（2026-07-14、実装前review反映）
...
2. 同仕様の `Thread Pane Contract / Pane header` と `Accessibility And Keyboard / Sources list` にある
   sources triggerの`aria-controls`付与契約を上書きする。`aria-expanded`は常に実表示と一致させ、
   `aria-controls`は対応surfaceがmountされたopen時だけ付与する。history toggleの契約は上書きしない。
3. completed を中心に定義されている stable presentation 契約を、failed
   （`errorCode=cancelled` を含む）と、その後の Server Component 収束までへ拡張する。
4. existing-thread submitのServer Actionとclient componentに重複しているrefreshを解消し、live terminalの
   client convergence ownerをrunごとに1つへ限定する。
5. local development の bind mount をコード反映保証とみなさず、標準の worker 再生成対象へ
   `worker-agent` を含める。
...
sources disclosure の初期値・所有権・trigger accessibility属性についてだけ本仕様を正とする。
...
7. `router.refresh()`は完了通知を返さないが、現controllerは呼出し直後から2秒retryを数えるため、遅いRSCと
   次attemptがoverlapしうる。仕様上のin-flight完了条件も定義されていない。
8. browser fixtureでDBをfailedへ更新してからfake SSE terminalを送ると、同時起動中のpollingが先にfailedを
   観測し、検証対象のSSE terminalを消費しないままtestが通る可能性がある。
9. suite開始時に1回だけseedするone-way active runをclosed / explicit-openの2variantで共有すると、先に
   `running -> failed`へ遷移したtestが後続testの前提を消費する。
10. closed時にsources triggerの`aria-controls`を省略する新契約と、常時付与を要求する親仕様の優先順位が
    冒頭の限定上書きに明記されていない。
...
- `frontend/src/features/research/hooks/useResearchRunLiveState.ts`: `router.refresh()`を完了ackなしのvoid callbackとして
  controllerへ渡す。
- `frontend/src/features/research/live/controller.ts`: EventSourceとpollingを同時開始し、terminal refresh直後に
  次のfinalization timerを予約する。
- `frontend/src/features/research/live/controller.test.ts`: retry間隔は固定するが、pending refresh中の非overlapを
  検証しない。
- `frontend/scripts/run-research-e2e.mjs`: Research fixtureをsuite開始時に1回だけseedする。
- `frontend/specs/research-workspace-layout-redesign.md`: sources triggerへ`aria-expanded`と`aria-controls`を
  付与する親契約を持つ。
...
- [Next.js `useRouter`](https://nextjs.org/docs/app/api-reference/functions/use-router) の`router.refresh()`は
  RSCを再取得・mergeするが、現在使用中のNext.js 16.2.10の`AppRouterInstance.refresh()`は`void`であり、
  request完了Promiseを公開しない。
- [React `useTransition`](https://react.dev/reference/react/useTransition) の`isPending`はAction開始から最終stateが
  表示されるまでtrueを維持する。本sliceは、このtransition終了を1回のrefresh attemptのcommit ackに使う。
...
10. existing-thread submitのServer Action成功後に同目的のclient refreshを重ねない。terminal収束のclient
    ...
    retry timerを作らない。
11. terminal refreshのretry delayは、専用React transitionがpendingからsettledへ変わり、RSC payloadが
    client treeへcommitされた後から数える。`router.refresh()`のreturnや呼出し時刻を完了条件にしない。
12. Playwrightのclosed / explicit-open terminal scenarioは別の固定runを使い、test順序やretryによらず
    `running`から開始できる。
13. sources triggerの`aria-controls`はunmounted surfaceや現在と異なるviewport modeのIDを参照しない。
14. 内部例外、provider payload、質問・履歴本文を API error や visible failure 文言へ公開しない。
15. REST API、Pydantic schema、generated TypeScript types、DB、認証・認可を変更しない。
...
- refresh完了通知用のbackend endpoint、SSE event、DB fieldを追加すること。
...
- terminal refreshが2秒を超えてpendingでも次attemptを開始せず、transition commit後にだけbackoffを開始する。
- pollingを固定active応答へ隔離したbrowser testで、fake SSE terminalがlive failureの入力になったことを確認する。
- closed / explicit-open用の独立fixtureが、それぞれのtest開始時にrunningへresetされる。
- sources triggerの`aria-controls`が限定上書きどおりmount済みsurfaceだけを参照する。
...
2. controllerが受け取るrefresh portはack付き非同期契約とし、`requestRefresh(): Promise<void>`のresolveを
   1 attemptのRSC commitとして扱う。現行の`refresh: () => void`契約は使用しない。
3. client adapterは専用`useTransition`内で`router.refresh()`を開始する。attemptは、このtransitionで
   `isPending=true`を観測してから`false`へ戻り、更新されたRSC payloadがclient treeへcommitされた時に完了する。
   `router.refresh()`のvoid return、呼出し直後、固定timeoutを完了ackにしない。
   専用backend ackは追加しない。
4. convergence成功はattempt完了とは別である。同じrunのpersisted terminalがRSC commitされ、active controllerが
   stop / unmountした時点を成功とする。owner dispose時は未解決ackを破棄し、retryを予約しない。
5. transition完了後も同じrunがactiveで、ownerがsubscribe中かつdocument visibleの場合だけ、その時点から
   2 / 4 / 8 / 10秒の既存backoffを開始する。delay満了時に次attemptを開始し、各attemptのcommit ackを待つ。
6. coordinatorは同時に1件を超えるpending attemptまたはretry timerを持たない。pending中のvisibility復帰、
   SSE、polling、timer callbackは追加refreshを開始しない。Reactが別transitionとbatchしてackが遅れることは
   許容するが、RSC commit前にackを早めない。
7. hiddenへ移った場合はpending attempt自体を重ねず、settle後もhidden中はtimerを予約しない。visible復帰時は、
   pendingなら何もせず、pendingがなく同じrunがactiveな場合だけ次attemptを開始する。
8. SSEとpollingが同じterminalを観測しても、同じcoordinatorへ収束し、重複したattemptを作らない。
9. terminal refreshはthread detailをDB正本へ収束させるsignalであり、live payloadからfinal answer、source、
   missing aspectsを合成しない。
10. RSC commit後はactive run controllerを停止してよいが、turn wrapper、answer scroller、composer、announcer、
    sources disclosure ownerを同じthreadで作り直さない。
11. terminal refreshが失敗・遅延した場合も、live terminal presentationを維持し、空のanswer panelへ戻さない。
...
| refresh transition adapter | 1回の`router.refresh()`とReact transition commit ack | terminal判定、retry policy、DB state |
...
11. controllerへdeferred refresh Promiseを渡し、terminal受理後に12秒進めてもcall countが1件である。
    1件目をresolveしてから1,999msでは1件、さらに1msで2件目となり、2件目pending中に10秒以上進めても
    3件目を開始しない。unsubscribe後にPromiseがsettleしてもretryしない。
12. refresh transition adapterは`isPending=true -> false`だけをcommit ackとし、pending中に同じrunから
    requestされても1件のPromiseを共有または拒否して`router.refresh()`を重ねない。
13. sourcesありのwide / compact初期closedでは`aria-expanded=false`、`aria-controls`なし、surfaceなしとする。
    explicit open時は`aria-expanded=true`となり、`aria-controls`が同commitでmountされたaside / sheetのstable IDと
    一致する。close / Escape / viewport跨ぎでは旧surfaceと属性を同時に除去する。
14. source 0件ではdisabled、`aria-expanded=false`、`aria-controls`なしを確認し、history toggleの既存
    `aria-controls` testは変更しない。

### Playwright

Playwrightのterminal continuityは実providerや実workerのraceに依存させず、次の決定的fixtureで検証する。

1. 既存`backend/scripts/seed_e2e_research.py`を、`continuity-closed`と`continuity-open`の2threadへ拡張する。
   両方が別の固定thread / message / run UUIDを持ち、過去completed assistant、`missing_aspects`、sourceと、
   2件目のuser message + `running` runを持つ。同じrunを2variantで共有しない。
2. 同scriptへ、固定variantだけを受けるreset commandと、対応runだけを条件付きUPDATEする
   `running -> failed/internal_error` commandを追加する。各test開始時に自分のfixtureをresetし、transitionは
   rowcount=1を必須とする。既存production guard、固定UUID allowlist、cleanupを維持し、任意run ID、任意SQL、
   production endpointは追加しない。
3. `frontend/scripts/run-research-e2e.mjs`はsuite前後のseed / cleanupを所有する。Playwright Node fixture helperは
   `closed | open`の固定enumに対するreset / fail commandだけをterminal同期点で実行し、外部LLMとprovider keyを
   使わない。このresetによりtest順序、retry、単独実行でも各scenarioを`running`から開始できる。
4. `page.addInitScript()`で対象fixture runのEventSourceだけを制御するfakeを導入し、他runへ影響させない。
   有効なattempt / generation / event IDを持つdraftとfailed terminalを、DB transitionとの同期点を明示して送る。
5. page mount前に対象runのpolling URLだけをinterceptし、fake SSE terminalを受理するまではDBを読まず固定
   `running / synthesizing` responseを返す。他runはcontinueする。これによりDB transition後もpollingが先に
   terminalを所有できず、failureのlive入力がfake SSEであることを固定する。
6. terminal送信前に同threadのRSC refresh requestをgateし、browserへMutationObserverと
   `requestAnimationFrame` samplerを注入する。stable turn配下の
...
3. closed / openの各testは対応fixtureをresetし、pollingを固定runningへ隔離してからpageをmountする。
4. draft表示とsampler開始後、allowlisted commandでDBをfailedへ遷移させ、続いてfake SSE terminalを送る。
   terminal受理前後でpolling由来のfailed responseが0件で、fake EventSourceがcloseされたことを確認する。
5. terminal RSC requestをgateしたままlive failureがpaintされたことを確認する。RSC responseを3秒以上保留し、
   2秒retry境界を越えても対象threadのRSC requestが1件だけで、failure表示が維持されることを確認する。
6. gateを解放してpersisted failedへ収束させる。解放後からpersisted commit完了までterminal RSC requestは
   累計1件のままとする。terminalからRSC commitまで各sampleでdraftまたは固定failureが常に1件だけ存在し、
   protected loading skeleton、document reload、source auto-openを発生させない。
7. persisted failed commitでturn anchor、clamped answer scrollTop、main panel、composerの位置が定義済み許容差を
   超えて跳躍しない。
8. `continuity-open`ではterminal前にsourcesを明示openし、refresh後もopen、source scrollTop、focusを維持する。
   `continuity-closed`は全遷移でclosedを維持する。
9. compact viewportではsheetを自動openせず、明示open時だけmodal / focus contractを適用する。
10. A→B→A navigationでは各threadをclosedで開始し、route cacheから過去のopen stateを復元しない。
11. console errorがない。
...
frontend/src/features/research/live/controller.test.ts
...
frontend/e2e/fixtures/research-runtime.ts                   # fixed reset / transition commands only
...
6. 2本のfixed active-run fixtureを使うPlaywrightでsource closed / open、failed convergence、visual continuityを
   独立して確認する。
...
2. frontend targeted test:
   - live controller / refresh transition adapter
   - `ResearchThreadView`
