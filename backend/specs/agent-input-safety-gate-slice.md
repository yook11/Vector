...
### 3. policy は「話題」ではなく「ユーザーが求める支援」を判定する

block は、現在の依頼が次の支援を明確に求める場合に限る。
...
次は話題に有害語が含まれても allow とする。

- ニュース、投資影響、法制度、歴史、研究、倫理の分析。
- サイバー攻撃、詐欺、市場操作等の検知、防御、予防、incident response。
- 有害な文章の要約、翻訳、分類。ただし新しい実行支援を加えない依頼。
- 自傷に関する支援要請、予防、相談先の質問。
- prompt injection や system prompt への言及だけを含む質問。
- 罵倒、下品な表現、オフトピックであるだけの質問。

判定は high precision を優先する。現在の依頼が実行支援を求めていると明確に判断できない
場合は allow とし、曖昧さ解消や通常回答時の安全制約は後続工程に委ねる。input safety gate は
全安全対策の代替ではなく、明確な違反を早期短絡する第一判定である。

### 4. 現在の質問と bounded thread history を入力にする

current question だけでは「その方法を成功させるコードを書いて」等の follow-up を判定できない。
そのため、既存の `AgentThreadRepository.read_recent_messages_before()` が返す最大6 message の
bounded history を参照して代名詞と依頼対象を解釈する。

- history は現在と同じ `seq < current user message seq` の窓を使う。
- prompt 投入時は各 message content を既存どおり最大2,000文字に制限する。
- assistant `missing_aspects` は安全判定に不要なため prompt へ渡さない。
- history は現在の依頼の参照先を理解するためだけに使い、過去 turn を再判定しない。
- 過去に危険な話題があるだけで current question を block しない。
- question / history は `sanitize_for_untrusted_block()` を通し、全て untrusted data として囲う。

history read は Context Resolver の起動ではない。DB read と input safety call は block 判定に必要な
前処理として許可し、LLM による context preparation は allow 後まで構築しない。
...
分類は2 field の小さい structured output であり、repair retry はコストと安全境界の複雑さに
...
4. build and call InputSafetyService
5a. allow:
...
- question / history 本文、その head、hash。
...
| cancellation wins race | existing cancel terminal | decisionに応じたblock auditのみ | do not start | existing cancelled state |
...
backend/app/queue/tasks/agent_run.py
  - history read 後、QuestionContextService より前に safety gate を実行
...
- question と history の境界タグ、section header が sanitize される。
- assistant `missing_aspects` を prompt に含めない。
- policy prompt が block / allow の各境界例を固定する。
...
- audit event に question / history / raw response が存在しない。
...
6. worker の history read 後に safety gate を配線し、block / failed の早期 return を追加する。
...
- audit / log / metric に question、history、raw response が含まれない。
