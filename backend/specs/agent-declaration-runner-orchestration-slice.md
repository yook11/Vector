...
本仕様は責任と移行順を固定する設計仕様である。`AgentDefinition` の最終的な Python 型、
instructions の同期・動的生成 API、model abstraction の具体型は、後続の Agent contract slice で
確定する。
...
- Agent 宣言は少なくとも stable name、instructions の正本、model 設定の正本、
  output type / schema の正本を関連づける。
...
- 初期 Agent 宣言に未使用の handoffs、guardrails、session、max turns、generic tool loop を
  追加しない。
- model が実行可能な Tool を直接選択しない初期設計では、`AgentDefinition.tools` を
...
- Agent / Tool runtime が取得した resource は、成功・typed failure・想定外例外・cancel の全経路で
  解放する。
- provider、retry、fallback、validation、audit、metrics、progress、delta、continuation の
  既存意味と順序は、明示的に変更する別仕様がない限り維持する。
...
- Agent phase / attemptとTool call spanにquestion、history、prompt、query text、candidate snippet、evidence本文、
  previous answer、final output を attribute として載せない。
...
- `AgentDefinition` の最終的な Python generic、dynamic instructions signature、model abstractionを
  本仕様だけで決定すること。
- 現行 prompt 文面、model、temperature、retry回数、provider、検索上限を変更すること。
...
- Agent phase / attemptとTool call span attributeにmodel-visible textや検索本文が含まれない。
...
Agent attemptは、1つのAgent definitionと型付きinputを`AgentRuntime.invoke()`へ渡し、1回のprovider
requestからschema parse済みdraftを受け取る単位である。streaming responseも開始からcloseまでを
1 attemptと数える。

### `AgentDefinition`

1つのLLM役割に必要なstatic declaration。stable name、instructions、model設定、output schemaを
関連づける。provider clientやrun-local stateを保持しない。

最終Python型は後続sliceで決めるが、少なくとも次を表現できなければならない。

| 項目 | 意味 |
|---|---|
| `name` | traceとdiagnosticで使うlow-cardinalityな役割名 |
| `instructions` | そのAgentだけが行う判断・生成の指示 |
| `model` / settings | 既存model specの正本への参照 |
| `input_type` | Runnerから投影される型付き入力 |
| `output_type` | validation済みの役割固有出力 |

### `AgentRuntime`

Agent definitionと型付きinputを1回のprovider callへ変換する共通実行境界。Runnerはrole phaseから
`AgentRuntime.invoke(definition, input)`を必要回数だけ呼ぶ。runtimeはattempt単位のprovider error
translation、schema parse、stream cleanup、attempt spanを所有する。
...
起動しない。compositionはdefinitionとは別にprovider-backed `AgentRuntime`実装をRunnerへ注入する。
...
Agent definition、Agent runtime、Tool、repository、reporterを `AnsweringRunner` へ配線する場所。
...
| `QuestionAnsweringAgent` | AgentDefinitionとRunner-owned role phaseへ分解 | workflow全体をAgentと呼ばない |
...
| `QueryGenerator` | External Search Query Agent definition / typed contract | LLM役割を明示する |
| `EvidenceSelector` | External Evidence Selector Agent definition / typed contract | LLM役割を明示する |
...
└─ agent_answering_run {run_id}                          # 唯一の回答Run
   ├─ question_context_phase {agent.name}                # LLM補助が必要な場合だけ
   │  └─ agent_provider_call {agent.name, attempt_number}
   ├─ question_planning_phase {agent.name}
   │  ├─ agent_provider_call {agent.name, attempt_number=1}
   │  └─ agent_provider_call {agent.name, attempt_number=2} # retry時だけ
   ├─ direct_answer_phase {agent.name}                   # direct時
   │  └─ agent_provider_call ...                         # attemptごと
   └─ [retrieval branch]                                 # 図上のgroup、spanではない
      ├─ internal_retrieval_call {capability.name}        # internal時
      ├─ external_query_phase {agent.name, task_index}    # taskごと
      │  └─ agent_provider_call ...
      ├─ external_search_tool_call {tool.name}            # queryごと
      ├─ external_selector_phase {agent.name, task_index} # taskごと
      │  ├─ agent_provider_call ...
      │  └─ agent_provider_call ...                       # retry時だけ
      └─ evidence_answer_phase {agent.name}
         └─ agent_provider_call ...                       # attemptごと
```

role phase spanはlogical phase / taskごとに1本、`agent_provider_call`は実provider requestごとに1本とする。
retryは同じphase配下のattempt spanを増やし、新しいphaseやRunを作らない。attempt usage / latencyは
`AgentRuntime`がattempt spanへ記録し、phase / run集計はそれらを二重計上せず合算する。

phase、attempt、Tool callはいずれもRun spanではない。図中の`retrieval branch`も実spanを要求しない。
`external_research_run` のような子Run名は追加しない。

許可する明示attributeの候補は、low-cardinalityなAgent / Tool名、attempt number、task index、
query count、candidate count、成功・分類済みfailure statusなどに限定する。次はattributeに含めない。

- raw question、history、previous answer、final answer。
- instructions、render済みprompt、provider response。
...
既存のPII / model-visible text非露出testを親spanからchild spanまで拡張する。
...
└─ agent/running.AnsweringRunner      # in-process workflow owner
   ├─ agent definitions / runtime     # LLM roles / provider attempt
   ├─ tool ports                      # search capabilities
   ├─ question context contracts
   ├─ planning contracts
   └─ answering / evidence contracts

agent definitions / AgentRuntime do not depend on
...
- Agent phase / attemptとTool callのusage / latency metricは、consumerとcardinality policyを定義する
  後続observability sliceで追加する。
...
- `AgentDefinition` / `AgentRuntime`を導入せず、`starting_agent`を削除しない。
...
- immutable `AgentDefinition`と、1 provider attemptだけを実行するnon-streaming
  `AgentRuntime.invoke()`を、最初の実consumerであるPlanner Agentと同時に導入する。
- Question Plannerのstable name、instructions、model spec、`QuestionPlanDraft` schemaをAgent宣言の正本へ移す。
- phase / attemptの語彙、typed input / draft output、schema parse、attempt spanをPlannerの実処理で固定する。
- 既存planning serviceのretry、fallback、finalization、audit、metricsをgeneric runtimeへ移さない。
- consumerを持たないgeneric runtimeや、Plannerが使わないstream API / cleanupを先行実装しない。
- Exit gate: `FLOW-02`、`FLOW-08`、`AT-07`、`OBS-02` / `OBS-05`のattempt部分と、既存planning
  retry / fallback / audit regressionを通す。
- 残すseam: `QuestionAnsweringOrchestrator`、workflow全体の`QuestionAnsweringAgent`、`starting_agent`、
  既存Generator / Selector port。削除するseam: Planner固有の重複したmodel / schema設定。

### PR2: External Query and Selector Agent contracts

- `DeepSeekQueryGenerator`と`DeepSeekEvidenceSelector`を、それぞれAgentDefinition + typed contractへ移す。
- structured-output用function schemaを実行可能なToolとして扱わず、Agent output schemaとして固定する。
...
- 残すseam: `ExternalSearchResearchRunner`と既存SearchProvider port。削除するseam: Query / Selector固有の
  重複したmodel / schema設定。
...
- phase / attempt / Tool span、text非露出、usage集約を完成させる。
- Exit gate: `RUN-02`、`OBS-01`〜`OBS-09`、`ARCH-01`〜`ARCH-03`と全regression matrixを通す。
- 削除するseam: `ExternalSearchRunResult`等の部分Run語と移行adapter。

各PRはbehavior-preservingを基本とし、prompt、model、provider、公開contractの変更を混ぜない。
例外はPR4で`ExternalResearchRuntime`（Query / Selector runtime client + External Search Tool client）の
activationをexternal branchまで遅らせるresource lifecycle改善だけである。credentialの開始API検証、
error shape、回答結果は変更しない。
...
- `RES-02`: direct / internal-only pathでQuery / Selector Agent clientとTavily clientを作らない。
...
- `OBS-02`: logical role phase spanをphase / taskごとに1本作り、実provider requestごとに同phase配下の
  `agent_provider_call`を1本作る。retryはattempt spanだけを増やす。
- `OBS-03`: `retrieval` groupや`external_research_run`等のchild Run spanを作らない。
- `OBS-04`: 親spanの明示attributeを`run_id`だけにする。
- `OBS-05`: child span attributeにquestion、history、instructions、prompt、query text、URL、title、
  source name、published at、snippet、claim、why selected、missing、previous answer、final outputの
  sentinelを含めない。
- `OBS-06`: Agent / capability / Tool名を固定low-cardinality値にし、input由来文字列を含めない。
- `OBS-07`: generated queriesは既存live event / task reportに残るが、trace、log、metric label、
  audit attributeへ出力しない。
- `OBS-08`: attempt usageをphase / runへ二重計上せず集約する。
- `OBS-09`: `AnswerGenerationStopped`をerror eventにせず、その他の貫通例外は既存error記録を維持する。
...
```text
backend/app/agent/running/                         # AnsweringRunner / AgentRuntime contracts
backend/app/agent/agent_definitions/               # 新規候補: static Agent declarations
backend/app/agent/question_context/                # Question Context Agent migration
backend/app/agent/planning/                        # Planner Agent migration
...
directory名はAgent contract sliceで既存package語彙と照合して確定する。新しい抽象化directoryを
先に作るのではなく、最初のPlanner Agent consumerが必要とする場所から導入する。
...
## Open questions for the next Agent contract slice

次は本仕様の責任を変更せず、型としてどう表現するかを決める。

1. `AgentDefinition[TInput, TOutput]`をdataclassとするか、Protocol + concrete declarationとするか。
2. dynamic instructionsにlocal contextのどのprojectionを渡すか。
3. 既存`*Spec`をAgent definitionそのものへ昇格するか、Agent definitionから参照するか。
4. sync / async instructionsを両方必要とする既存consumerがあるか。
5. common runtimeのschema parseと、role phaseのdomain validation / finalizationの型境界をどう表すか。
6. attempt usageをphase / run単位へ集約して公開するconsumerとmetric contractをどう定義するか。

これらを決める際も、Agentがworkflowを所有しないこと、Runnerが唯一のRunを進行すること、
External Search Toolがquery生成・evidence選別を所有しないことは変更しない。
