...
└─ agent_answering_run {run_id}                                # 唯一の回答Run
   ...
   └─ [retrieval branch]                                         # 図上のgroup、spanではない
      ...
      ├─ agent_phase {phase=external_query, agent_name}             # taskごと
      │  └─ agent_provider_call ...
      ├─ external_search_tool_call {tool.name}                    # queryごと
      ├─ agent_phase {phase=external_selector, agent_name}          # taskごと
      │  ├─ agent_provider_call ...
...
`agent_phase`の独自attribute allowlistはlow-cardinalityな`phase`と`agent_name`だけとする。
...
- Agentの分類済みinvalid responseは、provider-neutralな`AgentResponseInvalidError`へ統一する。defectは
  ...
  input、model output、自由文message、URL、任意のvalidation contextを含めない。
...
- `DeepSeekQueryGenerator`と`DeepSeekEvidenceSelector`を、それぞれ`Agent` + typed contractへ移す。
...
- `OBS-06`: `agent_phase`の独自attribute allowlistを`phase` / `agent_name`、`agent_provider_call`の独自attribute
  allowlistを`agent_name` / `attempt_number` / 分類済み`result`とする。API操作、provider、model、usageは
