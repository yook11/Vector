...
Runtimeとnon-streaming fakeにはstreaming capabilityを要求しない。compositionは共用Gemini activatorを
streaming scope factory portにも配線する。
...
| `ExternalSearchRunResult` | PR9でaliasなし削除済み | workflow内の受け渡しDTOを残さない |
| `ExternalSearchRequest` | PR9でaliasなし削除済み | workflow内の受け渡しDTOを残さない |
| `ExternalSearchRunner` | PR9でaliasなし削除済み | nested Runner / capability portを残さない |
...
これはmigration完了時の最終allowlistである。PR7のstreaming answer attemptだけは、既存観測との段階移行として
`prompt_version`を一時的に維持するが、新しいmetric labelへ複写しない。PR10でこの例外を削除し、最終allowlistへ
統一する。
...
- 削除するseam: Direct / Evidence Answer固有のgenerator adapter、call spec、generator protocol、重複設定。
  残すseam: `DirectAnswerFlow` / `EvidenceAnswerFlow`とdelta / continuation機構。
...
- 削除するseam: `ExternalSearchResearchRunner`、`ExternalSearchService`、
  `ExternalSearchRequest` / `ExternalSearchRunResult`、`ExternalSearchRunner` / Runner-facing external
  searcher port、`build_external_search_service()`と外部部分処理のnested Runner呼び出し。

### PR10: Naming and observability cleanup

- PR9で削除したexternal pipeline seamを除き、部分処理に残る`Run` / `Runner`語をconsumer inventoryに
  基づいて置換する。
...
- 削除するseam: PR9後にも残る部分Run語と移行adapter。
...
- `RES-07`: Agent stream resourceを正常・異常の両経路でcloseする。Flowが生成済みouter iteratorを1回、
  Runtimeが取得済みSDK iteratorを1回closeし、未生成 / 未取得ならそれぞれ0回とする。開始済みattempt spanは
  SDK closeの成否やcancelにかかわらず1回だけ終了する。
...
- `OBS-05`: migration完了時、child spanのattribute、event、status descriptionにquestion、history、instructions、prompt、
...
- `OBS-06`: migration完了時の`agent_phase`の独自attribute allowlistを`phase` / `agent_name`と、external Query / Selector
  ...
  標準attributeとして同じく比較対象外とし、Runtimeは明示設定しない。PR7のstreaming answer attemptでは
  `prompt_version`を一時的に許可し、PR10で削除する。
...
- migration完了時のdata exposure境界として、model-visible text、入力由来文字列、Prompt version、
  childへの`run_id`複写を禁止する。PR7のstreaming answer attemptだけは段階移行としてPrompt versionを
  一時許可し、PR10で削除する。
