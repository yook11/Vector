...
7. Redisへpublishできるのは取得成功後であり、全envelopeの `attemptEpoch` は1以上である。
...
- SSE接続時のqueued応答を204 / 409 / 待機のどれにするかの決定
...
後続SSE層はこのstatusを受けたら、固定済みの旧epochで読み続けない。接続を終了するか、
所有権を再確認してDBから現在のrun contextを再取得する。具体的なHTTP / reconnect契約は
SSE sliceで決める。
...
consumerが下書きを破棄する条件もmarkerの出現回数ではなく、epochが変わったことに限る。
...
| marker欠落・trimで境界を失う | 全envelopeのepochを比較し、marker位置に依存しない |
