...
1. **read projection の拡張**: `app/agent/history/projection.py` は**既存**
...
5. **`get_agent_history_session` に理由コメント**: `get_session`（request 1 tx の UoW）
...
```text
backend/app/agent/history/projection.py     (既存拡張: 詳細 message 写像を追加、
                                             result 写像を削除 — 前提整理 1)
backend/app/agent/history/mapper.py         (assistant message 行の構築を集約 — 前提整理 2)
backend/app/agent/history/repository.py     (list_threads / read_thread_detail /
                                             delete_thread / cancel_run を user_id 拘束で追加。
                                             read_run_for_user は slim 化)
backend/app/agent/history/types.py          (AgentRunErrorCode.CANCELLED 追加)
