...
1. **read projection の拡張**: `app/agent/threads/projection.py` と
...
```text
backend/app/agent/threads/projection.py (既存拡張: 詳細 message 写像を追加)
backend/app/agent/runs/result_mapper.py        (assistant message 行の構築を集約 — 前提整理 2)
backend/app/agent/threads/repository.py (list_threads / read_thread_detail /
