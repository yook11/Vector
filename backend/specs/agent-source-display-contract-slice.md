...
- `app/agent/evidence_collection/internal_search/article_search.py:26-35` — `InternalArticleContent` は
  summary / key_points を保持するが、これは synthesizer 向け `text` の素材であり、
  final source 表示契約にそのまま出す必要はない。
- `app/agent/evidence_collection/external_search/contract.py:257-266` — `ExternalSearchEvidence.claim` は
