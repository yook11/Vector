...
- `acquire_for_execution()` は queued / running のrunを再取得し、そのたびに
  ...
  timeoutは300秒であり、重複配送時には旧workerが新attemptの開始後に遅延eventを
  publishし得る。
...
2. `PreparedAgentRun` が持つ正の整数 `attempt_epoch` を、すべてのStream envelopeに載せる。
...
- prerequisiteの `attempt_epoch` 以外のDB schema migration、回答下書きのDB保存、
  consumer group、consumer ack。
...
```text
type:       event type
attemptEpoch: `PreparedAgentRun.attempt_epoch` を10進整数文字列でserializeした値（1以上）
payload:    JSON object
publishedAt: publisherがUTC ISO 8601で付与する診断用string
```

readerは共通envelopeのtype / attemptEpoch / payload fieldを先に検証し、`publishedAt` は
境界判定に使わないopaque stringとしてdatetime parseしない。epoch分類後、現epochだけevent別の
...
- `read_after(run_id, attempt_epoch, cursor)` は要求epochを必須入力とする。SSE endpointは
  所有権確認で得た現在のrunの正の `attempt_epoch` を渡す。0以下はcaller契約違反として
  Redis操作前に拒否する。
- cursorなしの初回readは`XRANGE`で最大4096entryを一度だけ読む。cursorありのreadは
  `XREAD COUNT 128`を**blockなし**で呼ぶ。
- readerは要求epochと一致する有効entryだけをID昇順で返す。小さいepochは旧workerとして
  skipし、大きいepochは `attempt_advanced` としてcallerへ通知する。
...
- readerの結果型は次を区別する。
  ...
  attempt_absent  # keyはあるが、要求epochもより大きい有効epochもない。非terminal
...
attemptの帰属はappend順序ではなくinteger epochだけで決める。したがって、最新attemptの
markerがtrim・破損・lazy retry重複しても、残存entryの `attemptEpoch` が要求epochと一致すれば
返せる。逆に、新attemptのmarker後に旧workerがappendした小さいepochのentryは返さない。
...
- event帰属はenvelopeのinteger `attemptEpoch` の大小比較で決める。marker位置やappend順序では
  決めない。
...
4. readerの結果型が `events`、`empty`、`stream_missing`、`attempt_absent`、
   `attempt_advanced`、`cursor_trimmed`、`unavailable` を区別する。
...
8. cursor付きfollow readで旧epochだけをskipした `attempt_absent` はcursorを進め、後着の
...
1. `acquire_for_execution()` は取得ごとにDBの `attempt_epoch` を原子的に1増やし、
   `RETURNING` の整数を `PreparedAgentRun.attempt_epoch` として返す。`started_at` は時刻管理用に
   同時更新し、terminal runは `None` のままである。
...
- prerequisiteのattempt epoch migrationを検証する。このtransport変更による
  `/gen-types`、frontend検証は不要である。

## Done

- run単位のRedis Streamへ、型付きeventを上限・TTL付きでbest-effort appendできる。
- readerがRedis Stream IDをcursorにして、要求epochと一致する有効eventだけを順序通り取得
  できる。大きいepochを境界非消費で通知し、ゾンビworkerの遅延publish、marker重複・破損・
  trimで旧attemptを混ぜない。
