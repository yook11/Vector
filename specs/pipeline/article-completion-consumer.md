...
イベント駆動化の大枠と、段階的に実装した境界を定義する。個別の失敗分類や時間設定を先に固定せず、合意した方針と実装時の検討事項を区別する。
...
- 結果`CompletionFailureDecision`は不変で、`action`（retry / close）、`retry_at`（任意のUTC日時）、`code`、`requires_investigation`を返す。対応済みエラーは既存CODE、HostBlockedErrorは`host_blocked`、分類対象外は`unknown`とする。
...
- 有効な未来日時は60分やSQSの可視性上限へ短縮しない。SQSでの実現方法は配送接続時に具体化し、実現できないまま尊重済みとは扱わない。[RFC 9110 Retry-After](https://www.rfc-editor.org/rfc/rfc9110.html#section-10.2.3)
