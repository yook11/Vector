...
Status: 共通HTTP・補完固有エラーの定義はPR #356で実装済み・未マージ（2026-09-13）。ローカルmainで共通HTTP定義とその変換を実装。補完固有の変換・正常終了型・ハンドラー・Consumer・AWS接続は未実装。個別の再試行分類とRetry-Afterの解釈は後続で具体化する。
...
エラーはログ基盤に依存せず、工程名を付けるためだけに共通HTTPエラーを補完エラーで包み直さない。工程ハンドラーが工程情報を補い、ログ・監査・メトリクス等の出力側が項目と形式を選択する。全フィールド・文字列表現・原因例外をそのまま送信する前提にしない。
