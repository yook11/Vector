...
Status: 共通HTTP・補完固有エラーの定義のみ実装済み（2026-09-13）。正常終了型・変換処理・工程側ハンドラー・Consumer・AWS接続は未実装。個別の再試行分類とRetry-Afterの扱いは後続で具体化する。
...
エラーのフィールドと意味はエラー側の契約とし、ログ基盤に依存しない。ログ・監査・メトリクスへの出力項目と形式は出力側が明示的に選び、エラーの全フィールドや文字列表現をそのまま送信する前提にしない。新経路への接続時に各出力先の扱いを確認する。
