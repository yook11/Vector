...
Status: Partially implemented (Phases 1-4 completed; Phases 5-6 remain)
...
1. **rule の単一所有**: 「Stage X に進める条件」が `ReadyForX` に閉じ、TaskやServiceに散らない
...
4. **構造保証**: 不変条件はDB制約とfactoryで構造的に強制する
5. **Repository protocol 経由の DI**: Domainはprotocolだけを知り、infrastructure詳細（SQLAlchemy session）を持たない
...
Pattern A' 後は `ReadyForEmbedding` が型で「Analysis 存在 + embedding 未生成」を保証して渡されるため、行不在は **Pattern A' 違反 = 上流バグ**。Phase 2では(A)で開始し、実データで必要性が確認された場合だけ(B)へ進化する。
...
- 隠さず即時失敗させ、dead letterとalertで可視化する
...
- logで`concurrent_write`を残して可視化する
...
| **C** analysis.extraction | Article 本文を AI で構造化 (translated_title / summary / entities) | Extraction 未生成 + Article が存在 + 本文 ≤200_000 char (Ready 不変条件、system hard cap) | `ReadyForExtraction`: article_id, original_title, original_content。`Field(min_length=1, max_length=200_000)` + `Field(gt=0)` で構造保証、Gemini 側で 20_000 char に truncate (adapter 内部) | `article_extractions.article_id` UNIQUE + ON CONFLICT DO NOTHING RETURNING。`exists_for_article` 実装済 |
...
| **E** embedding | Analysis に embedding を生成 (`ArticleAnalysis.embedding` カラムを UPDATE) | Analysis 存在 + `embedding IS NULL` | `ReadyForEmbedding`: analysis_id, text_for_embedding (`translated_title + "\n" + summary`) | **embedding は別テーブルではなく `article_analyses.embedding` カラム**。`UPDATE ... WHERE embedding IS NULL RETURNING id` で race を構造化 + DB CHECK 制約 `ck_article_analyses_embedding_consistency` で両カラム整合。`model_name` は run-time に embedder.MODEL から決定するため Ready に持たせない |
...
### 5.2 入口 task (2026-04-28 時点の設計案 — superseded)

本節の task 数・cron・routing は現行 runtime の契約ではない。現行の正本は
`backend/app/queue/schedule.py` と各 task decorator であり、Stage A は
`dispatch_high` / `dispatch_medium` / `dispatch_low` が cron、`dispatch_sources` は
admin 手動実行である。
...
slots=True)` ではなくBaseModelを選ぶ理由は、Phase 1で判明したTaskIQのformatter制約
(Issue #441 / #558)にある。
...
3. **session を Domain に渡さない**: Repository protocolだけを受け、session管理は呼び出し側(Task)の責務とする
...
DB制約とfactoryによる構造保証との関係: Pydantic validatorは**構築時1回**のみ実行され、構造
...
| E embedding | analysis_id, text_for_embedding | text_for_embedding は title + summary の派生 | `Field(gt=0)` で analysis_id 正、`Field(min_length=1)` で text_for_embedding non-empty |
| C analysis.extraction | article_id, original_title, original_content | すべて Article copy (BC 越境のため try_advance_from は kwargs 経路) | `Field(gt=0)` で article_id 正、`Field(min_length=1)` で title/content non-empty、`Field(max_length=200_000)` で system hard cap |
...
将来throughputやpayload sizeが問題になった場合はmsgpackへ差し替えられるが、
現時点ではJSONSerializerを維持する。
...
|---|---|---|
| 1 | D classification | `PersistedAnalysisId`, `PersistedRejectionId`, `Analysis.from_draft`, `Rejection.from_draft` — Phase 1 で完了 (PR #227) |
| 2 | E embedding | `Embedding.from_draft` (UPDATE WHERE pattern なので `PersistedEmbeddingId` は元々ない) — Phase 2 で完了 (PR #238) |
| 3 | C analysis.extraction | `PersistedId` (extraction 用)、`Extraction.from_result` — Phase 3 で完了 |
| 4 | F digest | `Skipped` Outcome、`SnapshotRepository.insert_if_absent` / `upsert` (削除して `save(snapshot, *, force) → WeeklyTrendsSnapshot \| None` に統合)、Service の `generate_for_week` / `generate_for_latest_completed_week` (削除して `execute(ready)` に縮退)、Service の `_completed_week_start_for` static method (`app/insights/snapshot/domain/week.py` の純関数に移管) — Phase 4 で完了 |
...
3. **C: analysis.extraction** (D と類似、shape を transfer しやすい。本文 200_000 char hard cap は Ready 不変条件、20_000 char truncate は extractor 内部の話で責務分離) — Phase 3 で完了
4. **F: digest** (入口 task pattern の確立。Stage A より構造が単純なので先に実施) — Phase 4 で完了
