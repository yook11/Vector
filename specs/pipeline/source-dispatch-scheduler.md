...
- DB障害・取得失敗は恒久的な原因でもSQSへ失敗を返し、アプリで取得全体を再試行しない。
...
- `ArticleAcquisitionConsumer.consume()`がDB上のID・名前・有効状態を確認する。無効・削除済みの終了状態は保存せず、再配送時には再確認する。有効ソースの名前を登録済み取得定義へ解決し、`ArticleAcquisitionService.execute(source_id)`へ渡す。
