...
HttpTransportFailureは失敗した段階と理由を持ち、到達可能性は段階から導く。到達可能性はその単一通信試行の情報で、過去の送信やconsumerの処理完了を保証しない。
...
ログ項目はevent_id、attempt_count、stop_reason、error_code、requires_configuration_fixと、該当時のerror_reason・transport_stage・transport_reason・phase・original_exception_type・classification_exception_typeに限定する。
...
送信前と受信本文の契約違反は`ArticleAssessedInScopeEvent.from_input`から型内部の`_event_invalid_from_validation_error`を使って同じ不正情報へ変換する。`AssessedEventInvalidError`は`invalid`にreasonと不変のissuesを保持し、受信側はこの例外をそのまま受け取る。送信側はreasonとissuesを`PublishEventInvalidError`へ対応付ける。項目は既知フィールドだけ、コードはmissing_required_field・invalid_type・invalid_value・unknown_field・unsupported_event_type・unsupported_schema_versionとする。未知キーは親のevent／payloadへ集約し、同一項目とコードを重複排除する。元の入力・自由文・ValidationErrorの原因連鎖は保持しない。送信側の停止判断とバッチ契約は変更しない。種別だけを先に判定する分岐は共有契約へ統合し、複数の違反がある場合も外側の構造・種別・バージョン・payloadの順で送受信の分類を揃える。
