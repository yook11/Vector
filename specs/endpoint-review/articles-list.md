...
- クエリパラメータの camelCase alias (`perPage`, `sortBy`) — フロントの型生成と整合している
...
| フィルタ | source / keyword / category | 上記 + cosine distance < threshold |
...
> **記録**: 旧 `impact_level` フィルタは 2026-04 に廃止され（impact_level 完全廃止）、本ドキュメント当時の「決定1: impact_level を完全一致に変更」は無効化された。`_IMPACT_LEVEL_ORDER` dict と CASE 式も削除済み。
...
- **バックエンドテスト**: 検索エンドポイントのテストを新規作成（`embed_search_query` を mock）。一覧テストから `q` 関連を除去
