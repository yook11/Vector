...
| 集計バッチ broker | **専用 `broker_digest` + `scheduler_digest`**(Phase 1B の `worker_agent` とは独立) | LLM 不要だが既存パイプラインの broker と queue を分離して耐障害性確保。9 service 集約後は `worker-insights` / `scheduler` container に honcho 同居 |
