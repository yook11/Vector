...
Vector に追加する週次定期実行エージェント機能の仕様。最終更新: 2026-04-27 (Phase 1A 設計確定に伴い snapshot 永続化 / CQRS Service 分離 / worker-digest 専用 broker / EntityName 仕様を反映)。
...
- **Snapshot 永続化 (`weekly_trends_snapshots`)**:
  ...
  - 手動投入用 CLI を `app/digest/cli/generate_snapshot.py` に提供(初回投入 / 緊急再生成、`--force` / `--week=...`)
...
- **インフラ**: **専用 `worker-digest` + `scheduler-digest`** を追加(LLM は使わないが broker を分離して既存パイプラインへの影響を遮断)。新 API key 不要
...
| 集計バッチ実行系 | **専用 `worker-digest` + `scheduler-digest`**(LLM 不要だが broker / queue を分離して既存パイプラインへの影響を遮断) |
...
| 集計バッチ broker | **専用 `broker_digest` + `worker-digest` + `scheduler-digest`**(Phase 1B の `worker_agent` とは独立) | LLM 不要だが既存パイプラインの broker と queue を分離して耐障害性確保。`docker-compose.yml` に追加 |
...
| 経路 | 責任 | 引数 |
|---|---|---|
| 月曜 00:05 JST cron (UTC 日曜 15:05) | `app/digest/tasks/snapshot.py` の `@broker.task` | なし (直近完了週を自動算出) |
| 手動 CLI | `app/digest/cli/generate_snapshot.py` | `--week=YYYY-MM-DD` (省略時は直近完了週) / `--force` (既存上書き) |
...
- [ ] `app/digest/domain/trend.py` — `EntityTrend` / `TopicTrend` / `NewEntity` (末端 VO) + `WeeklyCategoryTrends` (集約ルート) + `WeeklyTrendsBundle` (snapshot 形)
- [ ] `app/digest/repository/trends.py` — 3 集計 SQL + `count_source_analyses`
- [ ] `app/digest/repository/snapshots.py` — `find_latest` / `find_by_week` / `insert_if_absent` / `upsert`
- [ ] `app/digest/config.py` — `MIN_CURRENT=5` / `MIN_PREVIOUS=2` / `NEW_BURST_THRESHOLD=10` / `SMOOTHING=2` 等の定数集約
- [ ] `app/digest/application/snapshot.py` — `WeeklyTrendsSnapshotService` (Command 経路)
- [ ] `app/digest/cli/generate_snapshot.py` — 手動 CLI (`--force` / `--week=...`)
- [ ] `app/digest/tasks/broker.py` + `tasks/snapshot.py` — 専用 broker_digest + 月曜 00:05 JST cron
- [ ] `docker-compose.yml` に `worker-digest` + `scheduler-digest` service 追加 (`brokers.py` の既存パターン参考)
- [ ] PR-C デプロイ後に CLI で初回 snapshot を投入 (frontend デプロイ前の必須前作業)

**PR-D — Query 経路** (PR-C 完了後)

- [ ] `app/digest/application/query.py` — `WeeklyTrendsQueryService` (`AsyncSession` DI、`find_latest()` 専用)
- [ ] `app/digest/router/weekly_trends.py` — `GET /api/v1/weekly-trends` (`Depends(get_optional_user)`)
- [ ] `app/digest/schemas/weekly_trends.py` — camelCase レスポンス schema (`weekStart`/`weekEnd`/`generatedAt` nullable)
...
- [ ] `make pipeline-status` で `worker-digest` + `scheduler-digest` が見えることを確認 (Makefile 側の `WORKERS` 変数も更新要)
...
- [ ] `app/digest/` を Phase 1B スコープに拡張(`domain/ports.py`、`domain/digest_service.py`、`domain/report.py`、`adapters/`、`filters/`、`tasks.py`)
