...
| 4 | F digest | `Skipped` Outcome、`SnapshotRepository.insert_if_absent` / `upsert` (削除して `save(snapshot, *, force) → WeeklyTrendsSnapshot \| None` に統合)、Service の `generate_for_week` / `generate_for_latest_completed_week` (削除して `execute(ready)` に縮退)、Service の `_completed_week_start_for` static method (`app/digest/domain/week.py` の純関数に移管) — Phase 4 で完了 |
