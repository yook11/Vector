...
論点3-Y の `news_source` Rich 化（PR 2）を実装した直後に、`save` 意味論
（立場 A vs B）と命名（論点2-C-3）を実コード経験を踏まえて再評価した。
結果は以下の通り。
...
閉じる瞬間に自動的に flush + commit される。新規エンティティの作成と
削除は明示的に `Repository.create` / `Repository.delete` を経由する。

採用理由（PR 2 の実コード経験）：
- `services/news_source.py` の `activate_source` / `deactivate_source` は
  `source.activate(); return ...` の形に薄まり、立場 A の典型例として
  違和感なく書けた
- 「最初は DB 変更の経路がパッと見えない」という負担はあるが、これは
  UoW 依存を選んだ代償として許容範囲と判断
- 立場 A のデメリットは「規約の明文化」で打ち消せる

### `save` リネーム撤回

論点2-C-3 で `Repository.create` を `save` にリネームする命名決定をしたが、
**この変更は撤回する**。立場 A 下では `save` の意味は実質「新規エンティティの
永続化」だけに縮退するため、`create` で完全に十分。リネームは機械的な
PR コストだけが残り、価値を生まない。

「命名は機能の意味論が決まってから振る」原則に従えば、立場 A が決まった
瞬間に `save` の必然性は消える。Repository の生成系メソッドは
**`create` のまま** とする。

### 規約の置き場所

立場 A の規約を以下の3箇所に分散配置する：

1. **`docs/adr/004_unit_of_work_service_convention.md`** — 正式な意思決定
   記録。「人が読むメインドキュメント」。Context / Alternatives / Decision /
   Rationale / Consequences の ADR フォーマット
2. **`backend/app/dependencies.py` の `get_session` docstring** — コードを
   読んだ読者が「ここで UoW が回る」と気づける場所への簡潔ポインタ。
   ADR-004 への参照リンクを含む
3. **本ドキュメントの本セクション** — 議論経緯と決着の保存

`backend/CLAUDE.md` には書かない。理由は以下：
- AI のコンテキストを圧迫する
- 実装パターンが規約を体現していれば、グローバルコンテキストに常駐させる
  必要はない
- ADR は必要なときだけ参照すれば十分

### 論点3-Y の進捗

- **PR 2 (完了)**: `news_source` Rich 化。`activate` / `deactivate` を
  Model のドメインメソッドに移動
- **次の PR (本決着の実装)**: ADR-004 新規 + `get_session` docstring 補強 +
  本セクションの追記
- **`Keyword` の Rich 化**: 別途。現状 `repositories/keyword.py` /
  `services/keyword.py` は存在せず、状態遷移ロジック（PROVISIONAL →
  OFFICIAL → BLACKLISTED）も未実装。AI 自動キーワード検出の実装と
  同じタイミングで、本規約を前提に新規実装する
...
- 関連 memory: `feedback_layer_architecture`（3層分離、ドメイン例外で
  Service → Router 接続）
- 規約の正式記録: `docs/adr/004_unit_of_work_service_convention.md`
