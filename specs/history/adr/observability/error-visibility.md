...
1. **FastAPI の未知 500 が工程軸に乗らない (かつ生例外文字列が漏れている)。**
   `app/main.py:184` は `NotFoundError` / `DuplicateError` / `InvalidQueryError` の 3
   ハンドラのみで、汎用 `Exception` ハンドラが無い。router 内の想定外例外は default 500
   になり、`surface`/`failure_kind` 属性が無いため「どの API 工程で落ちたか」を運用分類で
   切り出せない。**加えて** (2026-06-26 調査・capfire 実測): 未処理例外が貫通する全 span
   (instrument_fastapi server span / instrument_sqlalchemy / instrument_httpx / worker の
   `logfire.span`) は OTel exception event の `message`/`stacktrace` と `status.description`
   に生 `str(exc)` を保持し、logfire scrubber は `exception.*` を `SAFE_KEYS` で、
   status.description も対象外で **scrub 除外**するため、任意 PII が Logfire backend へ
   verbatim 送信される。可視化 (PR4) の前に export 境界での redaction (PR-R) が要る。
...
def pipeline_stage_span(
    ...
    *,
    ...
    with logfire.span(_SPAN_NAME, **attrs) as span:
        try:
            yield span
        except Exception as exc:  # CancelledError 等 BaseException 系は失敗軸に載せない
            annotate_span_failure(span, exc)  # 共有 helper が分類属性を複写 (PR2)
            raise
```

PR2 の複写は共有 helper `app/logfire/failure_attrs.py` の `annotate_span_failure(span, exc)`
に一元化し、`failure_kind` / `code` / `retryability` / `error_class` (+ `failure_action` は
値がある時のみ、現状 drop_article) を焼く。`pipeline_stage` と `article_stage` の両 backstop が呼ぶ。
**貫通例外**は backstop で自動的に焼かれるが、**article_stage の握り潰し失敗** (AI 3 工程で
handler が `reraise=False` を返し正常終了する経路) は backstop に届かないため、task の
service-failure `except` 先頭で `stage.record_failure(exc)` を明示的に呼ぶ (no-override で
元の業務例外を最優先で残し、二次例外で上書きしない)。`pipeline_stage` の握り潰し (単発 run
では実態は `acquire_source` のみ) は **PR2b** で同じ仕組みを `pipeline_stage_span` にも持たせる。
...
| **PR2 (Step 2, B案)** | 共有 helper `failure_attrs.py` で span に `failure_kind`/`code`/`retryability`/`error_class` (+`failure_action` は値がある時、現状 drop_article) を複写。**貫通例外**は両 helper (`pipeline_stage`/`article_stage`) の backstop で焼く (CancelledError は `except Exception` で carve-out)。**article_stage の握り潰し失敗** (AI 3 工程の service 実行失敗) は task の `except` 先頭で `record_failure(exc)` を明示 (no-override・元例外優先)。`pipeline_stage` の握り潰しは PR2b | `app/logfire/failure_attrs.py` (新規) / `stage_span.py` / `article_stage.py` / `app/queue/tasks/{curation,assessment,embedding}.py` | 既存 enum を使うのみ |
| **PR2b** | `pipeline_stage_span` を最小 recorder (`PipelineStageSpan`) yield 化し no-override な `record_failure` を持たせ、backstop も record_failure 経由へ。`acquire_source` の service-failure `except` 先頭で `record_failure(exc)` を明示し、握り潰し (reraise=False) でも span に分類が乗る。completion/briefing/trend_discovery は再 raise で PR2 backstop 被覆済み、backfill per-item は別。制御フロー・retry は不変 | `app/logfire/stage_span.py` / `app/queue/tasks/acquisition.py` | 既存 enum を使うのみ |
| **PR3 (実装済み)** | `vector.audit.dropped` counter を追加し、`*_audit_dropped` 全 25 箇所 (13 ファイル) で `record_audit_dropped(stage)` を呼ぶ。stage は drop した監査行の stage に一致させる (briefing の 5 件は `logger.info`)。穴 2 を埋める。網羅性は静的 AST (except handler 単位の併設) で構造的に強制 | `app/audit/metrics.py` (新規) + 各 `failure_handling.py` / `app/queue/tasks/*.py` / `app/audit/stages/trend_discovery.py` | 既存 enum を使うのみ |
| **PR-R (例外 redaction, PR4 先行)** | 例外貫通で span に乗る生 `str(exc)` を export 前に `[redacted]` へ落とす横断 redactor。対象=exception event の `message`/`stacktrace`、`status.description`、および `ValidationError` の失敗入力を載せる `exception.logfire.data` (event 属性 + span 属性の両方)。`exception.type`/`status_code` は残す。logfire は `exception.*` を `SAFE_KEYS` で scrub 除外/不十分なため、`MainSpanProcessorWrapper` 下流を wrapper で包む (内部 API 依存、fail-fast + capfire 回帰でガード)。手書き `str(exc)` 補間と LogRecord 経路は射程外。穴 1 の **安全化前提** | `app/logfire/redaction.py` (新規) + `app/logfire/setup.py` の configure 直後 | enum 非依存 |
| **PR4 (要合意後)** | API 未知 500 に運用分類 (`surface=api` / `failure_kind=unknown` / `op`) を付け穴 1 を埋める。**PR-R (redaction) 済みが前提**。`stage` は付けない (pipeline 専用ドメイン語彙)。機構は未決 (a) 汎用 `Exception` ハンドラ + 現 server span へ属性付与 / (b) middleware・route wrapper | `app/exception_handlers.py` / `app/main.py` | **`surface` 語彙 + 機構の合意が前提** ([未決](#未決事項) #1) |

PR1〜3 は `Stage` enum に一切触れず、DB schema / API shape も変えないため独立に進められる。
PR-R は現存する PII 漏洩の封じ込めのため PR4 より優先する。PR4 のみ語彙設計を含むため後段に置く。
...
| PR2 | helper 単体 (capfire): 未知例外で `failure_kind='unknown'`/`code='unexpected_error'`/`error_class` が乗る + 実 marker で classified な値が乗る + `CancelledError` は失敗属性を載せない (carve-out) + `record_failure` の no-override。e2e (AI タスク): **握り潰し** (reraise=False, terminal drop) で `article_stage` span に元例外の分類属性 + `failure_action='drop_article'` (curation)、**貫通** (reraise=True, recoverable) で分類属性 + exception event の二層、**二次例外**で元の業務例外が保持されることを assert。両 `_ALLOWED_DOMAIN_KEYS` ([test_article_stage.py](../../backend/tests/logfire/test_article_stage.py) / [test_pipeline_stage.py](../../backend/tests/logfire/test_pipeline_stage.py)) に failure 5 キーを追加し、失敗パスでも allowlist 超過キー非含有 anti-test が緑 (I4) |
| PR3 | (1) 静的 AST で `except` handler 単位の drop-log × `record_audit_dropped` 併設を固定 (同一ファイル内相殺・`except` 外誤配置・大文字数字名を検知)。(2) helper 単体 (capfire): +1 / 属性 `stage` のみ / PII 非含有。(3) 配線 e2e: audit session を故意に壊し代表 site で +1 と stage を確認 (backfill は runtime stage のスレッドを固定) |
| PR-R | capfire で raw `exported_spans` を見る ((`exported_spans_as_dict()` は status.description を落とす)。(B) worker `logfire.span` と (A) instrument_fastapi server span の両方で、例外貫通後に `exception.message`/`exception.stacktrace`/`status.description` が `[redacted]` かつ PII マーカ不在、`exception.type`/`status_code=ERROR` 残存を assert。負のコントロール (install しない) で PII が残ることを確認 (teeth)。setup 不変条件で `setup_logfire` が `install_exception_redaction` を呼ぶことを pin |
| PR4 | 機構 (a) なら router で未知例外 → 500 + server span に `surface=api`/`failure_kind=unknown`/`op` 属性確認。機構 (b) なら子 span の **span 属性**確認 (語彙 + 機構合意後)。redaction (PR-R) 済み前提 |
...
- backfill の per-item 失敗 (バッチ内 1 記事を catch して loop 継続する経路) の集計・span
  複写は本シリーズでは扱わない (run 単位 span と性質が異なる別 PR)。
