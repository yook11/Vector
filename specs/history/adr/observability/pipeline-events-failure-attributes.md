...
`source_name` を解決できた範囲で保存する。`observed_article` の中身は
...
失敗の原因ファミリー。処理方針 (retry / hold / 記事削除) ではなく「何が起きたか」の
括りを表す。

例:

- AI 分析 stage (curation / assessment / embedding): provider 由来は AI provider の
  回復クラス (`AIProviderFailureMode`) の値 — `attempt_scoped` / `time_based_recovery` /
  `condition_based_recovery` / `operator_action_required` / `target_rejected`。
  応答が分析に使えない parse 由来は `ai_response_invalid`。
  処理方針 (retry / hold / 記事削除) は failure_kind ではなく、回復クラス由来の
  `retryability` 軸・handler の hold 導出・`failure_action` (curation の記事削除) が担う
...
- AI 分析 stage (curation / assessment / embedding) の provider 由来失敗で焼かれる
  (例: `timeout` / `server_error` / `safety` / `context_length`)。curation の記事削除
  (drop) 行にも、削除理由となった provider reason (`safety` / `input_blocked` 等) を残す
...
marker error class は write-time SSoT として失敗属性を持つ。retry 軸
(`RETRYABILITY`) と業務副作用 (`FAILURE_ACTION`) は型で固定し、原因軸
(`failure_kind` / `failure_reason`) は AI 分析 stage では instance 属性、
それ以外の stage では classvar `FAILURE_KIND` で持つ。
...
class CurationTerminalDropError(...):
    ...
    # code / failure_kind / failure_reason / provider_error は instance 属性
```
...
AI 分析 stage の Layer 1 marker (curation の `*RecoverableError` /
`*TerminalKeepError` / `*TerminalDropError`、assessment / embedding の
`*RecoverableError` / `*TerminalError`) は、原因軸 (`failure_kind` /
`failure_reason`) を **instance 属性** として持つ (型で固定するのは retry 軸
`RETRYABILITY` と、curation の記事削除副作用 `FAILURE_ACTION` のみ)。ACL の mapper が
provider error の回復クラス `FAILURE_MODE` から `failure_kind = mode.value` を、
`reason` から `failure_reason` を導出して詰める。marker が 3 本あるのは curation の
記事削除 (`FAILURE_ACTION=DROP_ARTICLE`) という業務 disposition を表すため。hold
(stage 退避) は marker 型ではなく handler が `provider_error.FAILURE_MODE` から導出する。
projection helper は `failure_kind` を instance 値優先で読み、briefing / completion /
acquisition の classvar `FAILURE_KIND` に fallback する。
