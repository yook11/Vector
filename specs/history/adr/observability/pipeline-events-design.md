...
- DB 上で別テーブルに完全に残るもの（analyzable_articles 本文全体など）
...
| `article_id` | int | NULL | — | FK analyzable_articles(id) ON DELETE SET NULL |
...
- AI 入力（記事本文 / サマリ）は `analyzable_articles.original_content` に残るが、**監査時点の値**は再抽出で書き換わる可能性
...
[第 2 防御] structlog で構造化ログ出力       ← 副、fallback、短期保持
              （業務エラー + 監査エラーを必ず両方出す）
  ↓ それも消えたら
[第 3 防御] analyzable_articles の "穴" として症状検知  ← 最終バックストップ
...
4. 業務テーブル（analyzable_articles）と紐付けたクエリが書けない
5. ダッシュボード化が困難
