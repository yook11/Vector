...
作成: 2026-09-24
Status: 仕様確定・テストを新しい契約へ移行済み (実装前のため失敗する)・実装未着手
...
| `aws_signed_query` | `redact_aws_signed_query_credentials` | `X-Amz-Signature` / `X-Amz-Credential` / `X-Amz-Security-Token` (大文字小文字不問) の `=` 以降、`&`・空白・引用符まで | 値だけを置換し、クエリ名と他のパラメーターを残す |
...
| `credential` | `redact_credential_assignments` | 正規化後に `CREDENTIAL_KEYS` と完全一致するキー名と、閉じ引用符・`=` または `:` | キー名と区切りを残し、その後ろを末尾まで置換する |
...
- ポリシーの `mask` は項目単位だけで働き、文字列の内容には影響しない。基底の認証キーは `deny` と漏洩防止が持つため、`BASE_MASK` は空にする。
...
| 3 | `leak_prevention.py` を作り、`sanitize.py` の検出処理と `mask.py` を移して削除する。値準備・診断を入口へ切り替え、`BASE_MASK` を空にする。 | 単体・適用箇所・出力後のテスト。 |
| 4 | `sanitize.py` の対応表を `sanitize_article_url` に置き換え、構築時の検証を追加する。 | サニタイズと規則構築のテスト。 |
| 5 | ベンチマークスクリプトの参照を更新し、関連仕様の記述を同期する。 | `/check` で変更範囲を検証する。 |
