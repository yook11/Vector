...
Watchlist 一覧で表示したいのは Dashboard と同じ `NewsBrief` の情報（翻訳タイトル、サマリ、impactLevel、キーワード等）。
...
| テスト | 変更内容 |
|---|---|
| `test_returns_watchlist_items` | `originalTitle` → `translatedTitle`, `summary`, `impactLevel`, `isWatched=True` をアサート |
...
- `NewsBrief` フィールドを使用（`translatedTitle`, `summary`, `impactLevel`, `keywords`）
