...
| PR8 retrieval dispatch ownership | 同上 | Planned |
...
| EC | internal/external/mixed dispatch | `EvidenceCollectionService` / `test_evidence_collection.py` | PR8で`AnsweringRunner` / Runner branch testへ移す | Recording/Scripted retrieval ports |
...
| ID | 保証条件 | 根拠 | 現production owner | 現test owner | 差し替え境界 | 判定 / 移行後 |
|---|---|---|---|---|---|---|
| EC-01 | internal/external/mixedを選択portへdispatchし、非選択portを呼ばず、mixedは両枝を並行実行する | FLOW-04/06/07 | `EvidenceCollectionService` | `test_evidence_collection.py` | Recording retrieval ports | 維持。PR8でRunnerへ移設 |
| EC-02 | classified片側failureを値へ変換して成功側根拠を保持し、unknownは両枝合流後に規則どおり伝播する | ERR-07〜10 | Service | `test_evidence_collection.py` | Scripted ports | 維持。PR8でRunnerへ移設 |
...
| EX-RES-01 | external branchだけでDeepSeek/Tavily resourceを共有activateし全経路closeする | PR4仕様 | 未実装 | なし | Fake SDK factories | gap（Planned）。PR4 factory、PR8 Runner activationへ |
...
| IS-08 | classified internal failureをcollection層で継続可能な値へ変換する | continuation仕様 | Internal＋Collection Service | 両service tests | 各層直下port | 維持。PR8で変換先ownerをRunnerへ |
...
| `evidence_collection/test_evidence_collection.py` | EC-01/02/03 | PR8 seatbelt |
...
| `evidence_collection/external_search/test_service.py` | EX-12 | PR4/8/9 seatbelt。EX-RES-01は未実装のplanned gap |
