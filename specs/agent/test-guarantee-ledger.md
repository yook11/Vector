...
| PR5 AnsweringRunner workflow ownership | `backend/specs/answering-workflow-ownership-slice.md` | Implemented |
...
| PR9 external pipeline ownership | `backend/specs/external-pipeline-ownership-slice.md` | Implemented |
...
| FLOW | Direct/Search branch・progress・phase timeline | `AnsweringRunner` / `running/test_answering_workflow.py`, `answering/test_orchestration.py` | owner不変 | Scripted phases、timeline recorder |
| RESULT | direct result と evidence citation・missing・status assembly | `AnsweringRunner._answer_directly`, `answering/result_assembly.py` / `answering/test_orchestration.py`, `answering/test_result_assembly.py` | owner不変 | なし |
...
| RUN | outer run・context・hook・run span | `AnsweringRunner` / `running/test_answering_runner.py`, `running/test_answering_workflow.py` | owner不変 | Scripted preparer/phases。spanは実Runner |
...
| PL-02 | DirectAnswerPlan と SearchPlan の排他、article query/task の各 1〜3 件、normalization、draft→completed plan | planning contract/spec | planning contract | `planning/test_contract.py` | なし | 維持 |
...
| PL-05 | response defectだけを同一 scope で最大 2 attempt まで retry し、最終 defect・provider error・unknown・cancel は plan を作らず伝播する | planner contract | `QuestionPlanningService` | `planning/test_planner.py` | Scripted runtime＋scope | 維持 |
...
| PL-08 | completed planまたは分類済み最終failureごとにoutcome metricを1回、planned/failed、plan type、retry、固定failure codeだけで記録し、scope失敗・unknown・cancelでは記録しない | planner observability spec | Service/metrics | `test_planner.py` | Scripted runtime | 維持 |
...
| ID | 保証条件 | 根拠 | 現production owner | 現test owner | 差し替え境界 | 判定 / 移行後 |
|---|---|---|---|---|---|---|
| EC-01 | SearchPlan は内部記事検索と外部リサーチを固定 2 枝で並行実行し、DirectAnswerPlan は両枝を起動しない | Search plan contract | `AnsweringRunner` | `running/test_retrieval_dispatch.py` | Recording retrieval ports | 維持 |
| EC-02 | classified internal failure は成功側外部根拠を保持し、非 cancellation の枝例外は join 後に規則どおり伝播する。outer cancellation は両枝を join 後に元の cancellation を再送出する | Search plan contract | `AnsweringRunner` | `running/test_retrieval_dispatch.py` | Scripted ports | 維持 |
...
| EX-03 | Queryは1 attempt。classified failure/timeoutでTool/Selectorを短絡し、unknownを伝播。queryをnormalize/capする | ERR-01、AT-01 | `AnsweringRunner` external private phase＋`external_search.policy.clean_generated_queries` | `running/test_external_pipeline.py`＋`external_search/test_policy.py` | Scripted runtime | PR9で最終ownerへ移設済み |
| EX-04 | Selectorは同じtyped inputで最大2 attempt、classified failureだけretryし、finalization errorをsafe defect化する | ERR-05 | `AnsweringRunner` external private phase＋external search policy | `running/test_external_pipeline.py`＋`external_search/test_policy.py` | Scripted runtime | PR9で最終ownerへ移設済み |
| EX-05 | candidate poolをround-robin、URL先勝ちdedupe、provider/pool capで構築し部分failureを継続する | REG-05/09 | `AnsweringRunner`＋external search policy | `running/test_external_pipeline.py`＋`external_search/test_policy.py` | Scripted runtime＋Fake Tool | PR9で最終ownerへ移設済み |
| EX-06 | Selector indexからsource metadataを復元し、範囲外/重複/cap超過をdrop・計数する | AT-06、REG-07 | `AnsweringRunner`＋external search contract/policy | `running/test_external_pipeline.py`＋`external_search/test_policy.py` | Scripted runtime＋Fake Tool | PR9で最終ownerへ移設済み |
| EX-07 | task並列上限を守り、全task/reportを順序どおり返し、1 task failureを隔離する | ERR-06、REG-04/08 | `AnsweringRunner` external private phase | `running/test_external_pipeline.py` | 専用concurrency probe | PR9で最終ownerへ移設済み。専用fakeは共有しない |
| EX-08 | Query/Selector phaseが実provider attemptを包含し、task indexはphaseだけ、model-visible textをtraceへ出さない | PR2、OBS-02/05/06 | `AnsweringRunner` phase＋DeepSeek Runtime | `running/test_external_pipeline_tracing.py` | 実Runtime＋Fake DeepSeek client | PR9で最終ownerへ移設済み |
| EX-09 | Tool portへclean queryとlimitを渡し、workflow backstop timeoutをclassified failureへ変換する | PR3、AT-01/02 | `AnsweringRunner` external private phase | `running/test_external_pipeline.py` | Fake/Blocking Tool | PR9でworkflow policyを最終ownerへ移設済み |
...
| EX-12 | effective count、empty短絡、cross-task URL dedupe、report/evidence/source ref accounting | external pipeline仕様 | `AnsweringRunner`＋external search contract/policy | `running/test_external_pipeline.py`＋`external_search/test_contract.py`＋`external_search/test_policy.py` | Scripted runtime＋Outcome capture | PR9で最終ownerへ移設済み |
...
| ID | 保証条件 | 根拠 | 現production owner | 現test owner | 差し替え境界 | 判定 / 移行後 |
|---|---|---|---|---|---|---|
| IS-01 | 正規化済み article query を value object として受け、blank/cap違反を拒否し、実embed文字列からhashを決定する | embedding仕様 | query value objects | query embedding tests | なし | 維持 |
...
| IS-08 | classified internal failureを Search の collection failure へ変換し、外部根拠の成功結果を保持する | Search plan contract | `AnsweringRunner` | `running/test_retrieval_dispatch.py` | Scripted internal search port | 維持 |
...
|---|---|---|---|---|---|
| RESULT-DIRECT-01 | direct answer の answered result は source/missing が空で、plan_summary.plan_type は direct_answer | result contract | `AnsweringRunner._answer_directly` | `answering/test_orchestration.py` | 維持 |
| RESULT-CITATION-01 | unknown citation refを拒否し、実在refだけをevidence順・重複なしでsourceへ射影する | result contract | `answering/result_assembly.py` | `answering/test_orchestration.py`, `answering/test_result_assembly.py` | 維持 |
| RESULT-REQ-01 | unknown requirement IDを拒否し、既知IDをcontext順のmissing文言へ変換する | result contract | `answering/result_assembly.py` | `answering/test_orchestration.py`, `answering/test_result_assembly.py` | 維持 |
| RESULT-MISSING-01 | evidence empty→collection failure→task missing→draft missing→requirement missing順で先勝ちdedupe | workflow仕様 | `answering/result_assembly.py` | `answering/test_orchestration.py`, `answering/test_result_assembly.py` | 維持 |
| RESULT-STATUS-01 | collection failure、missing、Search answered の source 不足で insufficient へ cap し、answered 時は missing を空にする | result contract | `answering/result_assembly.py` | `answering/test_orchestration.py`, `answering/test_result_assembly.py` | 維持 |
...
| CT-02 | `AnswerPlanSummary`が`plan_type`とcollection failureを保持し、direct answerとfailureの組合せを拒否する | `agent/test_contract.py` | 維持 |
...
| `running/test_external_pipeline_tracing.py` | EX-08 | 最終owner境界を実Runtime＋Fake SDK clientで保証 |
| `evidence_collection/external_search/test_policy.py` | EX-03〜06/12 | domain純関数の正本 |
| `evidence_collection/external_search/test_contract.py` | EX-12 | outcome validatorの正本 |
| `evidence_collection/external_search/test_external_search_tool_contract.py` | EX-11 | Tool adapter固有spanを維持 |
...
| `answering/test_orchestration.py` | FLOW-BRANCH/PROGRESS/INPUT/ERROR, RESULT-DIRECT/CITATION/REQ | `AnsweringRunner` と result assembly の実行経路保証 |
| `answering/test_result_assembly.py` | RESULT-CITATION/REQ/MISSING/STATUS | result assembly の純関数保証 |
