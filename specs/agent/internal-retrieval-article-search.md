...
- lookup / save failure は metric / log / audit で観測できるようにする。
...
2. internal retrieval 用に query を正規化し、最大3件に cap する。
3. query embedding cache を lookup する。
4. cache miss または lookup failure により cached vector が得られなかった query だけ `InternalQueryEmbedder` で embedding を作る。
5. 新規 embedding は best-effort で cache に保存する。
6. cache hit と新規 embedding を合わせた `InternalQueryEmbedding` を作る。
7. `ArticleVectorSearchRepository` で query ごとに記事を検索する。
8. 複数 query で同じ `article.curation_id` が出た場合は、最小 `distance` の hit を採用する。
9. 最終件数を cap して `list[InternalArticleSearchHit]` を返す。
