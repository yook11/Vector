...
`InScopeAnalyzedArticle` は assessment で生成されるが、用途は assessment に閉じない。保存済み analyzed article として、assessment の保存経路、embedding / internal retrieval の読み戻し経路、agent の内部検索 projection が共有する境界型である。そのため `backend/app/analysis/assessment/` 配下ではなく、`backend/app/analysis/` 直下に置く。
...
```python
class SearchableInScopeArticle(BaseModel):
    article_id: int
    article: InScopeAnalyzedArticle
    published_at: datetime | None
```

`SearchableInScopeArticle` は `embedding IS NOT NULL` の query 条件を通った検索可能 article である。`embedding` 自体は型に持たせない。検索可能性は repository query と戻り値型の組み合わせで保証する。
