...
> 等の **振る舞い変更は対象外** であり、後続のStage 4振る舞いリファクタPR群
> で扱う。
...
PR3.5-d.1 ┐ DB rename migration
          ...
          │   ranbook で管理。
          │
PR3.5-d.2 ┐ API / frontend follow-up (調査の結果、対象不在で実装ゼロ)
          │   API には Stage 4 文脈の `analysis` / `rejection` field 露出なし、
          │   WatchlistEntry column はユーザ視点の概念名として据え置き確定。
          │   spec 申し送り欄のクローズと WatchlistEntry コメント更新のみ。
          │
PR3.5-d.3 ┐ classify_content alias 削除 (✅ 完了)
          │   3 条件 (broker queue / 直近 24h alias_invoked ログ / DLQ すべて
          │   0 件) を運用側で確認した上で alias 関数 + alias テスト + module
          │   docstring の互換注記を削除。振る舞い不変。
          │
PR3.5-d   ┐ Stage 4 振る舞いリファクタ (本 spec の対象外)
          │   Layer 2-B 例外、AssessmentAuditRepository、record_assessment_failure、
          │   4-except dispatch など。後続PR群で実装。
```
...
## PR3.5-d.2 (調査記録 — API rename 対象不在につきクローズ)

PR3.5-d.0 / PR3.5-d.1 deploy 完了後の事前調査で、本 PR の前提が崩れたため
**実装作業ゼロ** で完結した。記録のみ残す。

### 調査結果

1. **Stage 4 文脈の API field `analysis` / `rejection` は露出していない**:
   PR3.5-d.0 (Python rename) で `Analysis` / `Rejection` Domain Entity を
   `InScopeAssessment` / `OutOfScopeAssessment` に rename した時点で、API schema
   層 (`app/schemas/`) には既に当該 field が露出していなかった。
   `app/insights/briefing/schemas/briefing.py::_StoryOut.analysis` は同名だが
   briefing story 内の AI 生成テキスト (`max_length=MAX_STORY_ANALYSIS_LEN`) で
   あり、Stage 4 InScope/OutOfScope とは **別概念**。rename すると意味が変わって
   壊れるため対象外。

2. **WatchlistEntry の column 名 `article_analysis_id` は据え置きで確定**:
   API 境界では `articleId` で露出 (`WatchlistCreate.article_id` を
   `alias_generator=to_camel` で camelCase 化)、内部 column 名は完全抽象化済み。
   ユーザ視点で「ウォッチした分析記事」を表す概念として `article_analysis_id`
   を保持する判断 (ユーザに見える概念に直結する column 名は rename しない)。

3. **frontend の `articleAnalysisId` は登場しない**: backend から JSON 露出が
   ない以上、generate-types 実行も不要。

### 結論

実装変更ゼロ。本 PR (PR3.5-d.2) は spec の申し送り欄クローズと WatchlistEntry
コメントの「整理予定」表現の更新 (=「据え置きで確定」) のみ実施。
...
- 2026-05-09 初版: Stage 4 命名統一計画を独立 spec として記録 (rename と振る舞いリファクタを
  PR レベルで分離する判断)。
- 2026-05-09 PR3.5-d.2 クローズ: 事前調査により API rename 対象は存在せず、
  WatchlistEntry の column 名はユーザ視点の概念名として据え置きで確定。実装変更
  ゼロのドキュメント整理 PR として完結。
- 2026-05-09 PR3.5-d.3 完了: PR3.5-d.0 deploy 後の運用ログで 3 条件 (broker
  queue 内 `classify_content` 0 件 / 直近 24h `classify_content_alias_invoked`
  0 件 / DLQ 0 件) を確認した上で、alias 関数 (`backend/app/analysis/tasks.py`
  内 27 行) と alias テストファイル
  (`backend/tests/analysis/test_assess_content_alias.py` 130 行) を削除。Stage 4
  命名統一の rename 系 PR 群 (PR3.5-d.0/d.1/d.2/d.3) はこれで完結。以降の
  Stage 4 振る舞いリファクタは後続PR群で追跡する。
