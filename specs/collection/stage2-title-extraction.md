...
- **型レベル保証**: `dict[SafeUrl, _]` のキー一意性により「URL 重複なし」を構造的に強制する
...
- 現行のStage 2実装: [`scraper.py`](../../backend/app/collection/article_completion/scraper.py) と
  [`completer.py`](../../backend/app/collection/article_completion/completer.py)
- 実データで検証してから廃止を判断するため、Stage 1 title収集の廃止はPhase Bまで保留する
- `articles.original_title NOT NULL`はStage 2のquality gateで構造的に守る
- 実装が似ていても解く問題が違えば共用しない。本文とtitleはどちらも分析入力の取得なのでStage 2で一貫して扱う
