          ...
          Condition = {
            ArnNotEquals = {
              "kms:EncryptionContext:aws:lambda:FunctionArn" = [
                ...
                "arn:aws:lambda:ap-northeast-1:123456789012:function:slice-test-auth-rate-limit-cleanup",
              ]
            }
          }
        },
    ...
    condition = (
      ...
      alltrue([for s in local.outbox_pass_role_guards : s.Sid != "DenyPassRoleToLambdaExceptPipelineRoles" ? true :
        ...
        toset(s.NotResource) == toset([
          ...
          "arn:aws:iam::123456789012:role/slice-test/slice-test-assessment-backfill-lambda",
          "arn:aws:iam::123456789012:role/slice-test/slice-test-embedding-backfill-lambda",
...
run "relay_boundary_allows_only_dedicated_db_user" {
  ...
  assert {
    ...
    error_message = "Embedding Relayの境界は専用DBユーザーだけに接続を許可し、旧Appユーザーへの接続を許可しない。"
  }
}
