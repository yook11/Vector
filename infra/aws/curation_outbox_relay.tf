...
resource "aws_lambda_function" "curation_outbox_relay" {
  count = var.curation_outbox_relay_image_digest == null ? 0 : 1

  function_name                  = local.curation_outbox_relay_name
  role                           = aws_iam_role.curation_outbox_relay.arn
  package_type                   = "Image"
  image_uri                      = "${aws_ecr_repository.this["backend"].repository_url}@${var.curation_outbox_relay_image_digest}"
  architectures                  = ["arm64"]
  memory_size                    = 512
  timeout                        = 120
...
resource "aws_scheduler_schedule" "curation_outbox_relay" {
  ...
  target {
    arn      = aws_lambda_function.curation_outbox_relay[0].arn
