"""投入Lambdaの版と3スケジュールの稼働状態を安全に引き継ぐ。"""
...
def resolve(state: dict, requested_digest: str, requested_state: str) -> dict:
    ...
    for resource in state["resources"]:
        ...
        if (
            ...
            or resource["name"] != "source_dispatch"
            or resource["type"] not in ("aws_lambda_function", "aws_scheduler_schedule")
        ):
            continue
        for instance in resource["instances"]:
            ...
            if resource["type"] == "aws_lambda_function":
                ...
                cadence = instance.get("index_key")
                enabled = attributes.get("state")
                if (
                    not isinstance(cadence, str)
                    or cadence not in CADENCES
                    or cadence in schedules
                    or enabled not in ("ENABLED", "DISABLED")
                ):
                    raise ValueError("stateのスケジュールが不正です")
                schedules[cadence] = enabled == "ENABLED"
    if len(images) > 1 or (schedules and not images):
        raise ValueError("stateのLambdaとスケジュールが不整合です")
    if schedules and (set(schedules) != CADENCES or len(set(schedules.values())) != 1):
        raise ValueError("3スケジュールの稼働状態が不整合です")
...
def main() -> None:
    ...
    try:
        ...
        parser.exit(
            1, "取得依頼投入のstateまたは指定値が不正なため設定を生成できません\n"
        )
