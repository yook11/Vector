...
SCRIPT = Path(__file__).with_name("resolve-source-dispatch.py")
CURRENT = "sha256:" + "a" * 64
UPDATED = "sha256:" + "b" * 64
        ...
        "resources": [
            ...
            },
            {
                ...
                "instances": [
                    ...
                    for cadence in ("high", "medium", "low")
                ],
            },
        ]
    }
...
class ResolveSourceDispatchTest(unittest.TestCase):
        ...
        for arguments, digest in [((), None), (("--digest", CURRENT), CURRENT)]:
            with self.subTest(arguments=arguments):
                self.assertEqual(
                    ...
                    {
                        "source_dispatch_image_digest": digest,
                        "source_dispatch_enabled": False,
                    },
                )
        ...
        for enabled in (True, False):
            with self.subTest(enabled=enabled):
                self.assertEqual(
                    ...
                    {
                        "source_dispatch_image_digest": CURRENT,
                        "source_dispatch_enabled": enabled,
                    },
                )
    ...
    def test_update_image_does_not_start_stopped_schedules(self):
        self.assertEqual(
            ...
            {
                "source_dispatch_image_digest": UPDATED,
                "source_dispatch_enabled": False,
            },
        )
    ...
    def test_explicit_state_changes(self):
        for enabled in (True, False):
            with self.subTest(enabled=enabled):
                ...
                self.assertEqual(
                    self.settings(result)["source_dispatch_enabled"], enabled
                )

    def test_invalid_or_inconsistent_state_never_emits_configuration(self):
        partial = deployed_state()
        partial["resources"][1]["instances"].pop()
        mixed = deployed_state()
        mixed["resources"][1]["instances"][0]["attributes"]["state"] = "DISABLED"
    ...
    def test_deposed_instance_does_not_replace_current_image(self):
        ...
        self.assertEqual(
            self.settings(self.resolve(state))["source_dispatch_image_digest"], CURRENT
        )
