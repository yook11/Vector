...
def main() -> None:
    ...
    if not isinstance(config, dict) or set(config) != expected_keys:
        raise ValueError("account.jsonにはサンプルと同じ3項目だけを指定してください。")
    ...
    if not re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9_.-]*", config["aws_profile"]):
        raise ValueError("aws_profileにはAWS CLIのプロファイル名を指定してください。")
    trust_pattern = (
    ...
    smoke = {key: config[key] for key in ("expected_account_id", "aws_profile")}
    backend = {
        "bucket": f"vector-test-tfstate-{account}",
        "profile": config["aws_profile"],
        "allowed_account_ids": [account],
        "assume_role": {
            "role_arn": f"arn:aws:iam::{account}:role/vector-test/bootstrap/vector-test-terraform",
            "duration": "1h",
        },
    }
    files = {
        "bootstrap.tfvars.json": json.dumps(config, indent=2) + "\n",
