...
Status: タスク1・2の定義はPR #356で実装済み・未マージ（2026-09-13）。ローカルmainでは、その共通HTTP定義を前提にタスク3のHTTP変換を実装。補完固有の変換・工程側ハンドラー・新経路への接続は後続タスクとする。
...
8. エラーはログ基盤に依存しない。工程ハンドラーが工程情報を補い、出力側が出力先に応じた項目と形式を選ぶ。所在を付けるためだけに共通HTTPエラーを包み直さない。
...
検証結果（2026-09-13）: Ruff lint・format成功、新規22ケースを含む単体6,842件成功、`make test-integration PYTEST_ARGS='-x -q'`でDB統合1,434件成功。一時DB・Redis・networkの削除を確認した。専用の型チェックコマンドは現行設定にない。ローカルmainに追加した共通HTTP定義は、PR #356の承認済みコミット`be07b4f39`と同一であり、補完固有の定義は今回取り込んでいない。
