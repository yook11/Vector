...
2. 新経路の重複除去を無効にし、別記事・過去試行から抽出を独立させる。記事内の繰り返しは許容し、独自のキャッシュ管理は追加しない。
...
### 配送スライス2: 新経路の重複除去を無効にする

Problem: 同一記事の再処理や共通段落を持つ別記事の処理履歴によって、今回の抽出素材が欠落することを防ぐ。

Evidence: 新経路はdeduplicate=Trueを指定し、実抽出器はプロセス内の履歴で段落・本文全体を除外していた。実HTMLを使った2ケースを先行追加し、いずれも変更前にArticleExtractionEmptyErrorで失敗することを確認した。

Invariants: 毎回のHTMLから素材を返し、既出の文章であることを理由に除外しない。記事内の繰り返しは許容する。HTTP・文字コード・日時抽出・品質条件・旧Taskiqの設定を維持する。

実装: 新経路のdeduplicateをFalseに変更し、補完ローカルテストのキャッシュリセット用fixtureを削除する。独自のキャッシュ生成・寿命管理・重複除去・依存追加は行わない。以前のキャッシュリセットによるテスト間分離は、このスライスで不要になる。

Non-goals: 重複除去以外の抽出品質改善、日時解析等のキャッシュの全面禁止、旧スクレイパー変更、SQS・Lambda配送ハンドラーやAWS設定の実装。

Done: キャッシュを途中で消さずに再抽出・別記事先行の2ケースが成功し、Ruff lint・format、単体（not integration）、DB統合、ローカルテストが成功する。一時テスト環境を削除し、ローカルmainに未コミットで保持する。

検証結果: 実抽出器の先行2ケースは変更前に失敗し、変更後はHTML抽出19ケースすべてが成功した。単体6,990件、DB統合1,475件、ローカルテスト101件、Ruff lint・formatが成功し、一時環境の削除を確認した。旧スクレイパー・Taskiq・依存ファイルに差分はなく、スライス2を完了してローカルmainに未コミットで保持する。
