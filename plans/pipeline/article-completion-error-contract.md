...
- 元の原因例外は後続の変換処理から例外チェーンで保持できる形とする。本文・ヘッダー全体・秘密情報を保持する汎用payloadは追加せず、エラー文字列表現にも自由文を自動展開しない。
...
- どちらも`VectorDomainError`を継承し、文字列表現へ出すのは固定の`CODE`のみとした。HTTP応答情報・本文断片・原因例外の自由文を文字列表現へ展開しない。
