...
> **変更履歴:** `news_sources.category_id` FK は A-1 で追加後、運用上の柔軟性のため A-4 マイグレーションで削除。ソースとカテゴリの紐づけは廃止し、AI タグ付けで全カテゴリ横断のキーワード候補を使用する方式に変更。
...
| ID | 判断事項 | 結論 | 根拠 |
|----|---------|------|------|
| D-1 | フィードとカテゴリの紐づけ | **廃止**（~~`news_sources.category_id` FK~~ → A-4 で削除） | 当初 1:N で設計したが、1ソースが複数カテゴリに跨るケースがあり柔軟性に欠けた。AI タグ付け（B-2）で全カテゴリ横断のキーワード候補を使用する方式に変更 |
...
> **変更:** `category_id INTEGER NOT NULL FK → keyword_categories.id` は A-4 マイグレーションで削除済み。
...
> **変更:** 当初のハイブリッド方式（ソースレベル大分類 + AI 小分類）は `category_id` 廃止に伴い、AI 全振りの方式に変更。

| レイヤー | 何を決定するか | コスト | タイミング |
|---------|--------------|--------|----------|
| AI レベル | キーワードタグ（全カテゴリ横断） | API コスト | Phase 4 分析時 |

- AI 分析時: `KeywordCategory` → `KeywordCategoryLink` → `Keyword` の経路で全カテゴリの候補キーワードを DB クエリし、AI が記事内容に基づきタグ選択
- `news_keywords` の INSERT ロジックは `ai_analyzer.py` に実装済み（B-2 完了）
- `source_id` が `None`（移行前の既存記事）の場合もキーワードタグ付けは実行される
...
> **後続変更:** `category_id` FK と `keyword_category.py` への Relationship は当初 A-1 で追加したが、A-4 マイグレーション（`a4_drop_news_sources_category_id.py`）で削除済み。
...
10. API ソース: `SourceType.API` + `api_endpoint` でディスパッチ。C-1 で Hacker News 対応済み。未知の `api_endpoint` はログ警告
...
- `category_id` は A-4 で削除済みのため、シードデータにも含まれない
- downgrade: `feed_url` で DELETE

| ソース | feed_url |
|--------|----------|
| TechCrunch | `techcrunch.com/feed/` |
| FierceBiotech | `fiercebiotech.com/rss/xml` |
| BioPharma Dive | `biopharmadive.com/feeds/news/` |
| The Quantum Insider | `thequantuminsider.com/feed/` |
| Cointelegraph | `cointelegraph.com/rss` |
| Yahoo Finance | `finance.yahoo.com/news/rssindex` |
| ITmedia | `rss.itmedia.co.jp/rss/2.0/itmedia_all.xml` |
...
5. `analyze_article()`:
   - `KeywordCategory` → `KeywordCategoryLink` → `Keyword` の経路で全カテゴリ横断の候補キーワードを DB クエリ（`category_id` 廃止後は `source_id` に依存しない）
...
- `NewsSourceResponse` — `category_id` / `category_name` なし（`category_id` 廃止後に削除済み）
...
`SourceTable.tsx`:
- テーブルカラム: Name, Type（badge）, URL/Endpoint, Status（Switch トグル）, Last Fetched, Actions（Edit/Delete）
...
- フォームフィールド: name, sourceType（Select: RSS/API）, feedUrl/apiEndpoint（sourceType で条件表示切替）, siteUrl, fetchIntervalMinutes（Select: 15分〜24時間のプリセット）
- カテゴリフィールドは `category_id` 廃止に伴い削除済み
...
| # | タスク | 対象ファイル | 状態 |
|---|--------|------------|------|
| C-1 | Hacker News API 統合 | `hacker_news.py`[NEW], `news_fetcher.py`, `config.py` | ✅ 完了 |
| C-2 | Alpha Vantage News API 統合 | 新規サービス | 未着手 |
| C-3 | フロントエンドフィルタ整理 | `NewsFilters.tsx`, `CategorySidebar.tsx` | 未着手 |
| C-4 | `FetchButton.tsx` 活用判断 | `FetchButton.tsx` | 未着手 |
| C-5 | `source` 表示のソース名対応 | `NewsCard.tsx`, `NewsDetail.tsx`, `watchlist/page.tsx` | 未着手 |

#### C-1: Hacker News API 統合 ✅ 完了

**対象ファイル:**
- 新規: `backend/app/services/hacker_news.py` — `HackerNewsClient` + `HNStory` dataclass
- 新規: `backend/tests/test_hacker_news.py` — 11 テストケース
- 変更: `backend/app/services/news_fetcher.py` — `SourceType.API` + `api_endpoint=="hacker-news"` ディスパッチ追加
- 変更: `backend/app/config.py` — `hn_api_base_url`, `hn_min_points`, `hn_hits_per_page` 追加
- 変更: `backend/tests/conftest.py` — `sample_hn_source` fixture 追加

**実装済み内容:**

1. **Algolia HN Search API クライアント** (`HackerNewsClient`):
   - `fetch_recent_stories(since_timestamp)` — API 呼び出し、`url=None` の記事をスキップ（Ask HN 等除外）
   - `fetch_and_save_stories(source, session)` — 記事取得→重複排除→DB 保存→`SourceFetchResult` 返却
   - `source.last_fetched_at` を Unix timestamp に変換して `numericFilters=created_at_i>{timestamp}` に使用

2. **重複排除**: RSS フェッチャーと同じバッチチェック方式（guid + url の事前照会）

3. **API レスポンスマッピング**:

| Algolia `hits[]` | → `NewsArticle` | 備考 |
|---|---|---|
| `objectID` | `guid` = `"hn:{objectID}"` | |
| `title` | `title_original` | |
| `url` | `url` | `null` → スキップ |
| `created_at` | `published_at` | ISO8601 → datetime |
| — | `description_original` = `None` | HN は description を持たない |

4. **ディスパッチ**: `news_fetcher.py` の `fetch_news_for_sources()` で `SourceType.API` + `api_endpoint == "hacker-news"` を判定し `HackerNewsClient` に委譲

5. **エラーハンドリング**: `HTTPStatusError` / `RequestError` → `SourceFetchResult(success=False)` で既存パイプラインと統一

6. **シードデータ**: Settings ページから手動追加:
   ```
   name="Hacker News", source_type="api", api_endpoint="hacker-news",
   site_url="https://news.ycombinator.com", fetch_interval_minutes=360
   ```
...
1. Settings ページで HN ソースを追加（`source_type=api`, `api_endpoint=hacker-news`）
2. Fetch News を実行し、worker ログで HN 取得を確認
3. `news_articles` に `guid LIKE 'hn:%'` の記事が保存されること
4. RSS ソースと URL が重複する記事がスキップされること
