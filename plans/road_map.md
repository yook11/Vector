...
|ニュースソース|RSS 7件 + Hacker News API + Alpha Vantage API|
|カテゴリ|10大分類 × 72小分類（キーワード）、サイドバードリルダウン実装済み|
|Billing|Tier 1 有効、予算アラート設定済み|
|Rate Limiter|REQUEST_INTERVAL=1.5s|
...
### 3E-1. 管理者ロールとアクセス制御（旧 F-3）

**現状**: User モデルに `role` / `is_admin` フィールドなし。全 API は認証ユーザー = フルアクセス。

実装方針:

- User モデルに `role` カラム追加（`admin` / `user`）
- ロールベースの認可ミドルウェア / デコレータ
- 管理者限定エンドポイント: `POST /news/fetch`、将来の管理系 API
- Fetch News ボタンを管理者ログイン時のみ Dashboard に表示
...
**前提**: 3E-1（管理者ロール・アクセス制御）が完了していること。
...
|9|3E-1|管理者ロール・アクセス制御|—|🔜 独立着手可|
|10|3F-1|AI 評価ジョブ設定・単体実行|#9|未着手|
|11|3F-2|モデル比較（A/B テスト）|#10|未着手|
|12|3B-3|ウォッチリスト強化|#5|未着手|
|13|3E-2|content_fetch_attempts リセット手段|#9|未着手|
