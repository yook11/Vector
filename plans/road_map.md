...
- [x] Phase 3B-1: 重複記事検出（バックエンド実装完了、フロントエンド未着手）
...
- 調査スクリプトで閾値を検証・調整
- フロントエンド: 「+N sources」バッジ表示、グループ展開 UI
...
|7|3B-1|重複記事検出（コサイン類似度）|#4 と並行|✅ BE完了（FE未着手）|
