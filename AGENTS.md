...
- 軽微・局所的な作業は main agent が直接行い、スコープが大きくコンテキストを圧迫する場合、または独立した作業を有意に並行できる場合のみ委譲する。
- 起動時は担当範囲・所有権・完了条件を明示し、実装系は対応するコードが成果物に含まれる場合に限る（frontend-ui-builder: UI、test-writer: テスト）。
- main agent は報告だけで採用せず、実際のコード・差分・根拠を Problem / Invariants / Done と照合し、必要な検証を行う。
