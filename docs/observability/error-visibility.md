...
| **PR-R (例外 redaction, PR4 先行)** | 例外貫通で span に乗る生 `str(exc)` (`exception.message`/`exception.stacktrace`/`status.description`) を export 前に `[redacted]` へ落とす横断 redactor。`exception.type`/`status_code` は残す。logfire は `exception.*` を `SAFE_KEYS` で scrub 除外するため、`MainSpanProcessorWrapper` 下流を wrapper で包む (内部 API 依存、fail-fast + capfire 回帰でガード)。穴 1 の **安全化前提** | `app/logfire/redaction.py` (新規) + `app/logfire/setup.py` の configure 直後 | enum 非依存 |
