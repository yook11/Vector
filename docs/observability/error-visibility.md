...
元の業務例外を最優先で残し、二次例外で上書きしない)。`pipeline_stage` の握り潰しは **PR2b**。
...
- `pipeline_stage` の握り潰し失敗 (例: `acquire_source` の非 retryable fetch で
  `handle_source_failure` が `reraise=False` を返し error dict を返す経路) の span への
  failure 属性複写は PR2 では扱わない (**PR2b** へ委譲)。audit repo 深部で projection が
  計算されるため、observability を durability 境界へ結合させる是非の論点を伴う。
