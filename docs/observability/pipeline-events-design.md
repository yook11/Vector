...
| **C: ランタイム文脈** | `attempt`, `duration_ms`, `trace_id`, 例外オブジェクト | 関数フレーム退出 | 完全に失われる | Task / Service / Repository |
...
3. `duration_ms` は **計測したフレームで使い切る**（成功時 = Service、例外時 = Task）
...
| `duration_ms` | int | NULL | — | Service 成功時 / Task 例外時の所要時間 |
...
ALTER TABLE pipeline_events ADD CONSTRAINT ck_duration_nonnegative CHECK (duration_ms IS NULL OR duration_ms >= 0);
```
    ...
    async def execute(
        ...
        *,
        ...
        async with self._session_factory() as session:
            ...
            await self._record_event(event_repo, outcome, attempt, duration_ms)
