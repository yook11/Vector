...
この見直しで自分にとって一番大きかったのは、失敗を検知した場所が、その「なぜ」まで伝える——そこまでが責任なのだ、という考えを持てたことでした。

失敗を定義するには、その部分が何をしようとしているのかを、もう一度考え直す必要がありました。理解をしなければ失敗を定義することはできない、理解をしてから実装をする大切さにも気づきました。

正直に言えば、この記録を、まだ改善には活かしきれていません。それでも、どのソースから取れた記事がどこで失敗したのか、AI の応答がなぜ弾かれたのかが、理由とともに残るようになりました。

これを積み重ねていけば、どのソースが分析に進める記事を安定して出しているのか、AI 処理のどの失敗が多いのかが見えてきます。
取得元の見直しや、プロンプト・スキーマの改善——次はこの記録を、改善につなげていきたいと考えています。


# 全体を振り返って

進めば進むほど、知らないことが増えていく感覚があります。この振り返りをしている中でも、まだ十分に設計できていない箇所や、最初に AI を使って設計していた頃の影響で命名が曖昧なままの箇所が目につき、問題は多いように思います。だからこそ、最初から自分の意思で設計しないと、後から取り返すのは難しいのだと学びました。

このアプリケーションを作り始めた頃は、もっと良いものになると漠然と想像していました。けれど実際には、著作権の問題で収集できるニュースが限られたり、AI の利用やデプロイにコストがかかったりと、さまざまな制約があることを知りました。何を作るのか、どのようなアプリケーションにするのか——その視点を最初から持っていなければならないのだと、作りながら学んでいきました。

設計も、作るものの構想も、結局は最初の意思から始まる。それが、この振り返り全体を通して一番強く残ったことでした。

とても長いものになりましたが、最後まで読んでいただきありがとうございました。これからも、学習を続けていきたいと思います。
