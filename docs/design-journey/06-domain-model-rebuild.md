...
Ready 型を見直したことをきっかけに、これまで自分が導入してきたほかの型にも問題があるのではないかと考えました。
...
class ObservedArticle(BaseModel):
    """分析にはまだ進めないが、後続工程で補うため取得したデータを残す型。"""
```
...
実際に取得するときは、このアダプターの `collect()` を呼びます。
...
本来、ソース固有の事情は、それぞれのソースクラス自身が持つべきものではないか——そう考えました。
...
ソースが自分の事情や補完方針を持ち、取得の流れは共通の処理（`fetch_articles`）に任せる。その分担で、設計は見通しよくなりました。
