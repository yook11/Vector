...
けれど、このアプリケーションのどの部分が本当に使ってもらう理由になるのか。ユーザーが価値を感じるものは何なのか？そこを正面から考えきれていませんでした。
...
```
不思議なことに、リポジトリのメソッドも API の設計も、これまで何度もレビューし、何度も手を入れてきた場所でした。
...
ここで、投資に関係のない記事をどう扱うかを考える必要がありました。
...
これは、単に値が欠けているよりも悪い状態でした。公開日時が、投資家にとってニュースの鮮度を判断する重要な情報であることを、捉えることができていませんでした。
...
取得するソースによって、最初から持っている情報は違います。RSS に本文がほとんど入っていないものもあれば、公開日時が欠けているものもあります。一方で、HTML のページ本文から、より正確なタイトルを取れる場合もあります。

そこで、ソースが最初に渡してきた情報をそのまま最終形として扱うのではなく、HTML からスクレイピングを行い、足りない情報を補ってから次の工程へ渡すように考え方を変えました。
...
その後の AI 分析の設計にも一貫してつながっています。

ただ、価値の中心に手を入れたことで、新たな問題も見えてきました。
このアプリでは、非同期パイプラインの中で記事の取得から AI 分析までを行います。あるとき、一覧画面の記事が更新されていないことに気づきました。

修正を行おうにも、どのステージで止まったのかが、まったく分からない設計になっていました。
そこを見えるようにしようとして監査ログを入れ始めたことが、次の第5幕につながります。
