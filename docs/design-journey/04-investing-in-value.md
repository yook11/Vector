...
海外の先端テックニュースを集め、翻訳し、AI で分析して、日本の投資家が読める形にする。けれど、その中のどの部分が本当に使ってもらう理由になるのか。ユーザーが価値を感じるものはなんなのか？そこを正面から考えきれていませんでした。

当時のメモにも、残っています——「どんなことを実現するための機能なのかを考えずに実装したものは、大体リファクタリングの対象になる」。
...
まず取得した記事から重要な情報を正確に取り出し、そのあとで、その情報をもとに投資家向けの判断を加えていくという流れがよいのではないかと考えました。
...
「投資判断に資する具体的な事象がない場合は対象外にする」「技術用語が出ているだけで投資価値があると判断しない」という基準を明確にし、
概念として分けられるようにしました。
...
ここで、「外部が提供する記事」と「AI に渡して分析する記事」は同じではない、と捉え直すようになりました。
...
取得するソースによって、最初から持っている情報は違います。RSS に本文がほとんど入っていないものもあれば、公開日時が欠けているものもあります。一方で、HTML 側のメタデータに日付が残っていたり、ページ本文からより正確なタイトルを取れたりする場合もあります。

そこで、ソースが最初に渡してきた情報をそのまま最終形として扱うのではなく、足りない情報を補ってから次の工程へ渡すように考え方を変えました。HTML から本文や公開日時を補い、その後はタイトルも HTML から取り直すようになります。
