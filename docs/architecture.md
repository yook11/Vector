...
collection では `pipeline:dispatch` を dispatch と lease sweep の control Stream とし、そこから `pipeline:acquisition` と `pipeline:completion` へ task を投入します。実行資源は分けず、1つの `broker_collection` が両 Stream を購読し、1つの Taskiq 共有 worker process（1 process、`--workers 1`、共有 concurrency 5）で acquisition / completion を処理します。

collection の Stream と consumer group state、retention（保持履歴）、lag / pending / 最古 enqueue age は stage ごとに分離して観測できます。一方、worker slot、concurrency、DB pool、prefetch と backpressure、process の failure domain（障害境界）は共有したままです。したがって一方の処理が重い場合は他方の実行枠にも影響し、キュー分割だけでは capacity や failure isolation を分離しません。

AI 分析では curation を `pipeline:curation`、assessment を `pipeline:assessment` という 2 つの論理 Stream に分けています。ただし実行資源はまだ分けず、1つの `broker_analysis` が両 Stream を購読し、1つの Taskiq 共有 worker process（1 process、`--workers 1`、共有 concurrency 10）で処理します。

Stream と consumer group state が分かれるため、ACK 済み履歴を含む retained entries（保持件数）、未配達の lag、配達済み未 ACK の pending、最古 enqueue age は curation / assessment ごとに独立して観測できます。一方、worker slot、concurrency、prefetch と backpressure、DB pool、process の failure domain（障害境界）は共有したままで、キュー分割だけでは優先度、公平性、failure isolation は独立しない設計です。assessment の遅延が継続した場合に、この観測結果を根拠として worker 分割を別途判断します。
...
backfill や再投入は hold gate により stage 単位で抑制できます。ただし curation / assessment は共有 worker のため、process fatal や共通依存の障害は両 stage に影響します。
