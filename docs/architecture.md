...
2. **不正な状態を作りにくくする設計**
   ...
   各処理の中で毎回 `if` を追加して状態を確認するのではなく、そもそも処理できない状態が次の層へ渡らないように設計しました。

　　具体的には、型、値オブジェクト、DB 制約、責務ごとのテーブル分離を使い、データの正しさを局所的な条件分岐ではなく構造で担保　　する方針にしました。

　　バグが入りやすい確認処理を分散させず、仕様変更にも強い設計にするための取り組みです。

3. **失敗を後から追える運用基盤づくり**

   記事の収集から分析までを非同期で処理しているため、失敗時にどの工程で何が起きたのかを把握しにくい課題がありました。

　　そのため、各工程で起きた重要な出来事を 監査ログ `pipeline_events` に記録し、処理の流れを追跡できるようにしました。
   これにより、失敗箇所や処理状態を後から確認しやすくしました。

   ただし、この仕組みを日常的な監視や復旧判断に十分つなげられているとはまだ言えません。

   今後は、ニュース取得元ごとの成功率や、収集・翻訳・分析など工程ごとの成功率を可視化し、どの失敗をアラート対象にするかを整理していく段階です。

   あわせて、失敗した処理の確認方法や、再実行が必要なケースの判断基準も整備していく必要があります。
...
この章では、実害を観測した対策ではなく、脅威を想定して入れた予防的な設計判断をまとめます。
基本方針は、**どこかが破られても、その影響をできるだけ狭い範囲に閉じ込めること**です。
...
ただし、この DB-backed limiter は厳密なアトミック加算ではないため、高並行下では取りこぼし得ます。また `auth.rateLimit` テーブルには TTL による自動削除がないため、IP ローテーションや分散攻撃で行が増え続ける可能性があります。現在は maintenance worker が 30 分ごとに 10 分より古い `auth.rateLimit` 行を小バッチ削除し、ログイン試行制限に必要な短期 counter だけを残す設計にしています。攻撃傾向を長期に見る必要が出た場合は、raw row の長期保存ではなく aggregate metric や CAPTCHA / account lockout を別途検討します。
