...
collection では `pipeline:metadata` を dispatch と lease sweep の control Stream とし、そこから `pipeline:acquisition` と `pipeline:completion` へ task を投入します。実行資源は分けず、1つの `broker_content` が両 Stream を購読し、1つの Taskiq 共有 worker process（1 process、`--workers 1`、共有 concurrency 5）で acquisition / completion を処理します。
